...
JSONのみ回答: {"verdict":"confirm"|"question"|"contradiction","reason":"何が問題か・竹内さんに何を確認すればよいかを60字以内で具体的に","prediction":"このナレッジを使う具体的な場面・お客様の発言例・AIの返信例を80字以内で推測"}`,
  ...
  for (const q of deduped) {
    ...
    const questionText = q.verdict === "contradiction"
      ? `[knowledge_id:${q.id}]
⚠️【確認お願いします】${q.title} について既存ルールとの関係を確認させてください
...
━━ AIの予測（参考） ━━
このナレッジは「${phase}の場面」で使うと思われます。
例えばこんな状況: ${q.prediction || "（予測できませんでした）"}

❓ 竹内さんへの質問
① このナレッジと既存ルールは「場面が違う使い分け」ですか？それとも片方を削除しますか？
② AIの予測（上記）はだいたい合っていますか？ズレている場合は正しい場面を教えてください
③ このナレッジの内容を修正する場合は、正しい内容を教えてください`
      : `[knowledge_id:${q.id}]
❓【教えてください】${q.title} の使い方を確認させてください
...
━━ 参考: 同フェーズの既存ルール ━━
${existingRules || "（まだ確定ルールなし）"}

━━ AIが判断できなかった理由 ━━
${reason}

━━ AIの予測（参考） ━━
おそらく「${phase}の場面」で使うと思われます。
例えばこんな状況・返信: ${q.prediction || "（予測できませんでした）"}

❓ 竹内さんへの質問
① AIの予測（上記）はだいたい合っていますか？
② どんな場面・タイミングで使いますか？（例：「お客様が○○と言ったとき」など）
③ AIに何をさせたいですか？（例：「〇〇を伝える」「〇〇を確認する」など）
④ 内容を修正する場合は、正しい内容を教えてください`;
