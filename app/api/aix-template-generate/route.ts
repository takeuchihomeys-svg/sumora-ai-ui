...
━━━━━━━━━━━━━━━━━━━━
【文章の書き方 — 単語羅列の禁止（最重要文体ルール）】
━━━━━━━━━━━━━━━━━━━━
・箇条書き・「・」区切り・読点だけで名詞を並べる単語羅列は本文に使わない
  NG例: 「バストイレ別・オートロック・2階以上です」「オートロック、モニター付きインターホン、浴室乾燥機、インターネット無料」
  OK例: 「ご希望のバストイレ別はもちろん、オートロック付きで2階以上という条件も叶うお部屋です！！」
・物件情報と顧客希望は「接続詞・理由節」で繋いだ自然な文にする（「〜なので」「〜はもちろん」「さらに」等で結びつける）
・1文ごとに意味のまとまりを持たせ、前後の文と自然につながるように書く（文の途中で改行しない・助詞を省略して名詞を直結しない）
・同じ結論フレーズ（「かなりオススメ出来るお部屋となります」等）を1通の中で繰り返さない（使うなら1回だけ）
・LINEでの会話として読んだとき違和感のない、話しかけるような口語的な文体にする
...
export async function POST(req: NextRequest) {
  ...
  const brainMetaSection = brainMeta
    ? `━━━━━━━━━━━━━━━━━━━━\n【🧠 Brain戦略 — 生成の方向性】\n━━━━━━━━━━━━━━━━━━━━\n` +
      ...
      (ngPropLabels.length
        ...
        : "") +
      `※上記のBrain戦略（行動パターン・顧客タイプ・潜在動機・主要トピック等）は情報の羅列ではなく、文章の訴求軸として内在化させること。項目名・ラベルをそのまま本文に転記したり、要素を1文ずつ機械的に消化して詰め込んだりせず、自然な文章の流れの中に溶け込ませること。\n` +
      "\n"
    : "";
  ...
  const userPrompt = [
    ...
    resolvedCustomerConditions || brainMeta?.property_search_params
      ? `※上記の顧客希望は「条件リスト」ではなく「訴求の根拠」として使うこと。条件名を「エリア・間取り・家賃」のように羅列転記せず、物件の特徴と顧客の希望を結びつけた自然な会話文（例:「ご希望の〇〇にぴったりで、〇〇という点でもご条件に合っています」）で言及すること。`
      : "",
