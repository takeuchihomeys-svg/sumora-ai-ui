...
[物件の最大の強みを1〜2点・簡潔に。お客様の希望条件に最も響くポイントを選ぶ。例：「家賃8万円台・2021年築浅」「築浅・室内綺麗」など。★「敷礼0円」「敷金礼金なし」は敷金が0円かつ礼金が0円の場合のみ冒頭に書いてよい（どちらか一方でも金額がある場合は絶対に書かない）★お客様が駅・エリアを希望していない場合は「〇〇駅徒歩〇分」をここに入れない]、[お客様名]にかなりオススメ出来るお部屋となります！！
...
・「建築中」「新築未完成」「竣工予定」など内覧不可の物件の場合：「※こちらのお部屋は建築中のため、[竣工・入居予定時期]のご入居となります！！」のみで締める。案内誘導文（「お気に召されましたら〜ご案内させて頂きます」）は絶対に付けない
  【🔴 建築中物件の日付フォーマット（必ず守ること）】
  ・西暦（年）を含めない。「○月○日」または「○月」のみ（「2026年9月1日」→「9月1日」「2026年9月」→「9月」）
  ・入居時期比較の追加文は不要。「〇月のご入居をご希望の場合は〜」「ご希望の〇〇のご入居をご希望の場合は〜」などの比較・謝罪文は一切書かない
...
・【🔴🔴 最優先・絶対禁止】物件資料の「礼金」欄に1円でも金額がある場合（例：礼金110,000円・礼金1ヶ月等）は「敷金礼金なし」「敷礼0円」「初期費用をかなり抑えて」「初期費用を抑えられる」等の表現を冒頭一行・サマリー文・オススメポイントのどこにも絶対に使わない。画像で「敷金: なし」と見えても「礼金: 〇〇円」があれば使用禁止。この禁止は他のどのルール・例文より優先する。
・「敷金・礼金なし」「初期費用を抑えられる」系の文言は【敷金が0円かつ礼金が0円の場合のみ】使う。敷金のみ0・礼金のみ0の場合は使わない
・【🔴 絶対禁止】礼金に金額がある場合（礼金1ヶ月・礼金〇〇円等）は、オススメポイントに礼金・敷金関連の記述を一切入れない。「礼金1ヶ月のみ」「敷金なし」「礼金低め」「（敷金なし）」等の部分的アピールも禁止。敷金・礼金をオススメポイントに書いてよいのは【敷金0円かつ礼金0円の場合のみ】。
...
JSONのみ: {"greeting":"①","report":"②（[物件名]解決済み・改行は\\nで）","type_desc":"③","cta":"④またはnull"}`;
    ...
    // ── 🏢 管理会社に確認した（退去予定日・入居可能日・初期費用・駐車場・ペット飼育・設備・募集状況）＋ 近隣月極駐車場確認 ──────────
      ...
      const guidanceClose = guidanceType === "申込"
        ? `${name}さん良ければ先にお申込みでお部屋を抑えて内覧もできます😊！！良ければお申込みはいかがでしょうか！！`
        : guidanceType === "内覧"
        ? `${name}さんご都合よろしいお日にちにご内覧のご案内させて頂きます😊！！`
        : null;
...
${guidanceClose ?? `${name}お気に召されましたら[内覧解禁日]以降でご案内させて頂きます😊！！`}`,
