...
お手隙の際にご査収ください😌！！${sendExamplesText}`;
      ...
      message_text = await callClaude(sendSystem + sendDiffNote + sendStarNote, userParts.join(""), currentAction);
    ...
    // ── 🔍 内覧へ！ ──────────────────────────────────────────────
      ...
      if (rescheduleMode) {
        const rescheduleDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite);
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
      ...
      message_text = await callClaude(system + greetingTimeNote + viewingDiffNote + viewingStarNote, `${name}への内覧お誘いメッセージ。${propNamePart}${vacancyPart}${calendarPart}${templateStructureNote}${recentHistory}`, currentAction);
...
${templateText}`;
        ...
        if (image_url || estimate_image_url) {
          ...
          message_text = await callClaudeVision(checkSystem + checkDiffNote, content, currentAction);
        } else {
          message_text = await callClaude(checkSystem + checkDiffNote, userText, currentAction);
        }
      }
...
${SMORA_COMMON_RULES}`;
    ...
    // ── ✅ 確認します ──────────────────────────────────────────────
      ...
      const ackDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.acknowledge_check);
...
・余計な説明や長文は不要。完成したLINEメッセージのみ出力${greetingTimeNote}${ackDiffNote}`;
    ...
    // ── 📣 追客する ──────────────────────────────────────────────
      ...
      const [followupDiffNote, followupStarNote] = await Promise.all([
        getDiffKnowledgeForState(AIX_ACTION_TO_STATES.followup_revive),
