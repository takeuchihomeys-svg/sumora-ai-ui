...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
      ...
      message_text = await callClaude(system, `${name}への内覧お誘いメッセージ。${propNamePart}${vacancyPart}${calendarPart}${templateStructureNote}${recentHistory}`);
...
・LINEでそのまま送れる完成文のみ出力`;
        ...
        message_text = await callClaude(confirmSystem, `${name}への申込確定メッセージ。${property_name ? `物件名:${property_name}。` : ""}${recentHistory}`);
...
${examplesText}`;
      ...
      message_text = await callClaude(system, userMsg);
