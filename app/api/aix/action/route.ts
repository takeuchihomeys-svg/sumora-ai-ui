...
1. 本人確認書類（運転免許証・マイナンバーカード等）→ 表面・裏面の2枚が必須
2. 保険証（国民健康保険・社会保険等）
3. 収入証明書（直近の給与明細・源泉徴収票等）
4. 申込フォームの未記入・不完全項目の追記
...
・判断基準：
  ...
  - 2枚以上 → 本人確認書類（表・裏）は揃っている可能性が高い → 本人確認書類は依頼しない。3枚目以降は保険証・収入証明の可能性として手順3で考慮する

手順3. 保険証・収入証明書の送付状況を確認する
・お客様の発言に「保険証」「免許証」「マイナンバー」「給与明細」「源泉徴収」等の言及があれば、その文脈で判断する
・言及も該当画像もなければ未着と判断して依頼リストに含める
・画像の枚数が本人確認2枚を超えている場合、超過分は保険証等が届いている可能性があるため「揃っている」側に倒す（届いている書類を再依頼するのが最悪のミス）
...
・フォーム返送が見つからない、[画像]の対応関係が判断できない等、不足物を確信を持って特定できない場合は、標準セットとして「本人確認書類（表面・裏面）」「保険証」を依頼する
・推測で断定は書かない
...
　例：
　・本人確認書類（裏面）
　  ※免許証・マイナンバーカード等裏面の写真をお送りください
　・保険証
　  ※国民健康保険・社会保険どちらでも大丈夫です
...
${SMORA_COMMON_RULES}`;
        ...
        const rawDocsText = await callClaude(docsRequestSystem + appDiffNote + appStarNote, `${name}への書類依頼メッセージ。${recentHistory}`, currentAction);
        // JSONパース → コンポーネント結合
        try {
...
${isSimple
  ...
  : `{"movein_date":"入居日安心（①・任意・入居希望日の話が出ていなければ空文字）","invite":"内覧案内（②）","appeal":"物件アピール（③）","cta":"申込み推奨（④）","reassurance":"不安解消行（任意・なければ空文字）"}`}
