...
③締め：「気になる点ございましたらお気軽にお申し付けください！！」`
          : `【メッセージ構成 — この順番を厳守】
①入居日安心（任意）：お客様が会話の中で入居希望日・入居時期を述べている場合のみ、メッセージの冒頭に置く。下記【入居希望日と30日確保ルール】で計算確認したうえで「8月7日〜10日のご入居で問題ございません！！」のように断言して安心させる。入居希望日の話が出ていない場合はこの行を書かない（空文字にする）
...
${!isSimple ? `【入居希望日と30日確保ルール — 最重要】
・スモラでは「お申込み日から30日間」お部屋を確保できる。今日申込みすれば${todayJSTFmt}の30日後までは家賃発生なしでお部屋を抑えられる
・お客様が入居希望日を述べている場合は、必ず次の計算をする：「${todayJSTFmt} ＋ 30日 ＝ 確保期限」→ 確保期限が入居希望日と同じ頃またはそれ以降なら問題なし
　例：今日が7月9日・希望が8月7日〜10日 → 7月9日申込なら8月8日まで確保可能 → ほぼぴったりなので問題なし
...
・【絶対禁止】審査期間の話（「審査に3〜10日かかる」「逆算して早めのお申込みを」等）は一切書かない。入居日の話はすべて30日確保ルールだけで説明する` : ""}
