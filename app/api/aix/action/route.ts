...
${SMORA_COMMON_RULES}`;
    ...
    // ── 💰 見積書送る ─────────────────────────────────────────────
    // ※ 差分学習ルール注入について: estimate_sheet はOCR（JSON抽出）＋テンプレート組み立て式で
    //   自由生成のsystemプロンプトが存在しない。OCRプロンプトへの注入はJSON出力を壊すリスクがあるため対象外。
    //   将来AI自由生成に切り替える場合は getDiffKnowledgeForState(AIX_ACTION_TO_STATES.estimate_sheet) を注入すること。
...
数値は整数のみ（円・¥・カンマは除く）。不明な項目は0または空文字。
{
  ...
  条件から主なポイント（エリア・家賃・間取り等）を自然に組み込む`
      ...
      const sendDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.property_send);
...
お手隙の際にご査収ください😌！！${sendExamplesText}`;
      ...
      message_text = await callClaude(sendSystem + sendDiffNote, userParts.join(""));
    ...
    // ── 🔍 内覧へ！ ──────────────────────────────────────────────
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const viewingDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite);
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
      ...
      message_text = await callClaude(system + viewingDiffNote, `${name}への内覧お誘いメッセージ。${propNamePart}${vacancyPart}${calendarPart}${templateStructureNote}${recentHistory}`);
    ...
    // ── ✋ 申込へ！ ──────────────────────────────────────────────
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const appDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.application_push);
...
・LINEでそのまま送れる完成文のみ出力`;
        ...
        message_text = await callClaude(confirmSystem + appDiffNote, `${name}への申込確定メッセージ。${property_name ? `物件名:${property_name}。` : ""}${recentHistory}`);
...
${examplesText}`;
      ...
      message_text = await callClaude(system + appDiffNote, userMsg);
...
不明はnull。`;
    ...
    // ── 📍 待ち合わせ（時間なし → LINEから自動抽出） ───────────────
      ...
      // ※ 差分学習ルール注入について: condition_hearing はAI生成を使わない固定テンプレートのため注入対象外。
      //   将来AI生成に切り替える場合は getDiffKnowledgeForState(AIX_ACTION_TO_STATES.condition_hearing) を注入すること。
...
・「！！」を文末に使う・3行のみ出力`;
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const meetingDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.meeting_place);
...
・構成・文言は一切変えず [時間] だけを置き換えること`;
      ...
      message_text = await callClaude(system + meetingDiffNote, `会話履歴から待ち合わせ時間を読み取り、メッセージを生成してください。${recentHistory}`);
