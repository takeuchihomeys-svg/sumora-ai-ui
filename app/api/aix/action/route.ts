...
[物件の最大の強みを1〜2点・簡潔に。例：「横堤駅徒歩11分の新築物件」「敷礼0円・家賃8万円台」「駅徒歩5分・築浅物件」など。お客様の希望条件に最も響くポイントを選ぶ]、[お客様名]さんにかなりオススメ出来るお部屋となります！！

（オススメポイント）
・家賃[金額]円（管理費別の場合は「家賃[金額]円・管理費[金額]円（合計[金額]円）」の形式）
・間取り：[間取り名]（[LDKの広さ]、[洋室1の広さ]・[洋室2の広さ]…の順で記載）
・[路線名]「[駅名]」徒歩[X]分（お客様の希望徒歩分数以内なら記載。例：お客様が徒歩15分以内希望→13分でも記載OK。異なる路線でも駅名・徒歩分数が同じなら1行のみ記載。駅名に「駅」の字は付けない（例：「堺筋本町駅」→「堺筋本町」））
・[物件固有の強み1]
...
・ペット飼育可はお客様がペット可を希望・言及している場合のみオススメポイントに記載する。ペットについて一切言及がない場合はオススメポイントに書かない
