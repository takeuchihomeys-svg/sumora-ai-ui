  ...
  "commission_tax": 仲介手数料消費税（なければ0）
      ...
      try {
        const [coverDiffNote, coverStarNote] = await Promise.all([
          getKnowledgeForState(AIX_ACTION_TO_STATES.estimate_sheet, currentAction, conversationId),
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数・見積書の金額の繰り返しは禁止）${greetingTimeNote}${coverDiffNote}${coverStarNote}`;
  ...
  条件から主なポイント（エリア・家賃・間取り等）を自然に組み込む`
      ...
      const [sendDiffNote, sendStarNote, compPickupNote, compInviteNote, compCalendarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.property_send, currentAction, conversationId),
...
{"intro":"挨拶行（1行のみ）","pickup":"ピックアップ行（条件説明・1行）","vacating":"退去予定文（複数あれば改行で連結・なければ空文字）","invite":"内覧誘導文（なければ空文字）","calendar":"内覧日時（⑤ルールに従い通常は空文字・過去ルールで日時記載傾向あり時のみ「直近ですと〜ご案内可能です！！」）","closing":"お手隙の際にご査収ください😌！！"}`;
    ...
    // ── 🔍 内覧へ！ ──────────────────────────────────────────────
      ...
      if (rescheduleMode) {
        const rescheduleDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite, currentAction, conversationId);
...
${SMORA_COMMON_RULES}`;
      ...
      const moveInDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result, currentAction, conversationId);
...
・完成したLINEメッセージのみ出力（候補複数・前置きは禁止）`;
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const mgmtDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result, currentAction, conversationId);
...
・3行以外の追加は一切しない。解説・絵文字・補足は不要`;
        ...
        // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
        const beforeDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing, currentAction, conversationId);
        ...
        const afterDiffNote = AFTER_FIXED_TYPES.includes(after_type ?? "")
          ? ""
          : await getKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing, currentAction, conversationId);
...
${SMORA_COMMON_RULES}`;
    ...
    // ── ✅ 確認します ──────────────────────────────────────────────
      ...
      // ★宛先はお客様ではなく物件の管理会社・オーナー。name は既に「さん」付きのため、
      //   顧客名は familyName から「姓＋様」で組み立てる（${name}様 だと「〇〇さん様」と二重敬称になる）
      const ackCustomerLabel = familyName ? `${familyName}様` : "お客様";
      const ackSystem = `あなたは賃貸仲介サービス「スモラ」のLINE担当です。
物件の管理会社またはオーナー宛に送る「募集状況の確認 ＋ 最大限割引した初期費用の御見積もり依頼」メッセージを1つ生成してください。
※これはお客様宛ではなく、物件を管理している不動産会社・オーナー宛のメッセージです。宛先を取り違えないこと。

${SMORA_COMMON_RULES}

【ご案内中のお客様】${ackCustomerLabel}（このお客様のご入居に向けて物件の確認を行う）
【確認内容の文脈】会話履歴・物件資料から「どの物件について・何を確認するのか」を正確に読み取ること

【メッセージ構成（この順番・4〜6行程度でコンパクトに）】
1. 書き出し: 「${ackCustomerLabel}のご案内をしております！！」から始める（「お世話になっております」は使わない）
2. 確認内容: 会話・物件資料から読み取った物件名・号室・確認事項を正確に記載する
   例:「アルグラッドザ・タワー心斎橋○○号室の募集状況をご確認頂けますでしょうか！！」
   ※物件名・号室は会話や物件資料から読み取れたものだけを使う。読み取れない場合は「お送り頂きました物件の」とし、物件名・号室は絶対に創作しない
3. 初期費用の御見積もり（★必ず含める・スモラの核心的価値提案）:
   「あわせて最大限割引した初期費用の御見積もりもお願いできますでしょうか！！」
4. 締め: 「ご確認頂けますと幸いです！！何卒よろしくお願い致します！！」

【禁止事項】
・「お世話になっております」は使わない
・「承りました」「少々お待ちください」「確認中です」は使わない
・物件名・号室・家賃を創作しない（会話から取れない場合は「お送り頂きました物件の」を使う）
・長い説明・物件詳細・比較・勧誘・内覧誘導は書かない
・🙏 の絵文字は絶対に使わない

【出力】完成したLINEメッセージ本文のみを出力する（JSON・見出し・注釈は不要）${greetingTimeNote}${ackDiffNote}`;
