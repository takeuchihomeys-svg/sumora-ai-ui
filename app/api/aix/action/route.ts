...
type PropFacilityData = {
  ...
  //   種類は GuarantorType（independent/credit/shinyou/unknown・2026-09-26 に3種類へ）。旧の "licc"・「LICC系」と旧クライアントの日本語（独立系・信用系）も読む
...
KTIレジデンス西中島Ⅱ
退去しましたのでお部屋ご内覧可能です😊！！
〇〇さんご都合よろしいお日にちにお部屋ご案内させて頂きます！！
お気軽にお申し付けください😌！！

例3（退去予定日あり・○日以降可能）:
フジパレス西加賀屋 305号室現在募集中となります！！
7/11日退去予定のお部屋で7/12日以降でお部屋ご案内可能です！！
〇〇さん7/12日以降のご都合よろしいお日にちにご案内させて頂きます😊！！
...
・**「明後日」は使わない**（日付と曜日だけで書く）。使ってよい相対の語は「本日」「明日」だけ
  （2026-09-19 竹内・この事例。実送信188通で「明後日 M/D」は18通＝9.6%。候補日が飛んでいる時に相対の語が入ると連続した日程に見えて紛らわしい）
...
{"property_name":"物件名（読み取れなければ空文字）","company_name":"保証会社名（資料に書かれている会社名をそのまま・社名が無ければ空文字＝推測しない）"}`;
      ...
      const typeDesc =
        guarantorType === "独立系"
          ...
          : guarantorType === "信用系"
          // 2026-09-26 竹内さんの定義「信用系は金融系の情報ではなく過去の家賃滞納やトラブルがなかったかみられるばしょ」の言葉だけ（緩い・厳しいは書かない・全保連／ジェイリース等の旧 LICC系もここ）
...
{"message":"〜（実際のLINEメッセージ全文・改行は\\n で）"}`;
      ...
      try {
        ...
        giCustoms = ((giRows ?? []) as Array<{ name?: string | null; type?: string | null }>)
          .map((r) => ({ name: String(r.name ?? "").trim(), type: normalizeGuarantorType(r.type) ?? "unknown" }))   // 登録済みの旧 "licc" は信用系
          .filter((r) => r.name);
