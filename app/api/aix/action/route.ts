...
・【🔴 絶対禁止】「敷金なし」「敷金なし・即入居可能」など敷金だけに言及する表現はオススメポイントに絶対書かない。敷金・礼金が両方とも0円・なし・ーの場合のみ「敷金礼金なし」としてオススメポイントに記載可。片方でも金額がある場合は敷金・礼金に関する記述を一切書かない
・「敷金礼金なし」と書く場合は必ず資料の敷金欄・礼金欄が両方とも0円・なし・ーであることを確認してから記載すること。敷金がある物件に「敷金なし」「敷金礼金なし」と書くのは絶対禁止。礼金がある物件も同様に絶対禁止
・【🔴 絶対禁止】「管理費なし、礼金なし」「礼金なし・管理費なし」など管理費と礼金を並べて初期費用の安さを表現することは絶対禁止。初期費用の安さをサマリー文・描写文に書く場合は「敷金礼金なし」（敷金・礼金ともに0の場合のみ）のみ使用可。礼金なし+敷金ありの場合はサマリー文・描写文に初期費用の安さは書かない
...
【構成 — この順番を厳守】
①受け止め：会話の文脈に合わせた一言（定例文にしない）
  - 内覧希望が来た → 「かしこまりました！！」
  - 物件を気に入った → 「お気に召して頂けて嬉しいです！！」
  - 複数物件を比較中 → 「かしこまりました！！」
②ご案内の意思：会話から物件名が読み取れる場合は物件名を入れる＋「ご案内させて頂きます！！」
  - 物件名あり → 「〇〇マンションご案内させて頂きます！！」
  - 物件名不明 → 「お部屋ご案内させて頂きます！！」
${calendarNote ? `③日程：以下フォーマットで列挙（★改行して書く）
  直近ですと
  M/D（曜日）HH:MM〜HH:MM
  M/D（曜日）HH:MM〜HH:MM
  ご案内出来ます！！
④締め：「[お客様名]ご都合如何でしょうか！！」← 必ずお客様名（さん付き）を先頭に、「？」は絶対に使わず「！！」で終わる` : `③締め：「[お客様名]ご都合如何でしょうか！！」← 必ずお客様名（さん付き）を先頭に、「？」は絶対に使わず「！！」で終わる`}
...
6/30 15:00に幸田マンション幸町
現地エントランスお待ち合わせで何卒よろしくお願い致します！！
住所: 大阪府大阪市浪速区幸町3丁目7-8

【絶対禁止】
・「いかがでしょうか？」など「？」で終わる締め → 必ず「！！」
・①②③④をひとつの文にまとめない（改行して構成を守る）
