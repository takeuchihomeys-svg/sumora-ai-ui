...
・[路線名]「[駅名]」徒歩[X]分 ← 徒歩10分以内の場合のみ記載。11分以上は絶対に書かない
・[物件固有の強み1つ（例：インターネット無料・礼金0円・ペット可・築浅など）]

[物件の家賃・間取り・築年など主な魅力を1文でシンプルにまとめるサマリー文。「家賃管理費込[金額]円の[間取り]、[築年]築の築浅物件となります！！」など]

[間取りの各部屋の広さを描写（LDK○帖と開放感のあるリビングに、洋室○帖・洋室○帖と各室広々〜など）。続けて設備（オートロック・宅配BOX・浴室乾燥機・システムキッチン・インターネット無料など）を列挙して「設備面も充実しております！！」で締める。]
...
・（オススメポイント）は3〜4項目に絞ること（多くても4項目まで）
...
・オートロック・宅配BOX・管理人巡回などセキュリティ設備はオススメポイントに書かない（設備説明パートに書く）
・オススメポイントに書いた内容を設備説明パートで重複させない（例：インターネット無料をオススメポイントで書いたら設備説明では省く）
・徒歩11分以上の駅情報はオススメポイントに絶対書かない
