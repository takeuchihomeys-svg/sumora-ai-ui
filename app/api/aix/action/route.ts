...
${SMORA_COMMON_RULES}`;
    ...
    // ── ✅ 確認します ──────────────────────────────────────────────
      ...
      const ackSystem = `${GENERATION_SYSTEM}
${SMORA_COMMON_RULES}

【このメッセージの用途】お客様の依頼・質問に対して「今から確認します」という旨を伝える、確認宣言のLINEメッセージを1つ生成する。
【最重要 — 会話に合わせる】会話履歴から「お客様が何を確認してほしいのか」を正確に読み取り、その対象を具体的に本文に入れること。対象のない汎用的な「確認いたします」だけの空返信・固定テンプレ的な返信は禁止。
・物件の空室・募集状況を聞かれた／物件URL・画像が送られた → 「[物件名]の募集状況確認させていただきます！！」の形で物件名を必ず入れる（会話・物件資料から読み取れた物件名のみ使用。分からなければ「お送り頂きました物件の」とする。物件名は絶対に創作しない）
・費用・退去予定日・ペット可否・設備・交渉など管理会社への確認が必要な事項 → 「[確認内容]について管理会社に確認させていただきます！！」の形で確認内容を具体的に入れる
・確認宣言のあとは「確認出来次第ご連絡させて頂きます！！」で締めるのが基本形
【文量・トーン】
・2〜4行の簡潔な確認宣言に留める。長い説明・物件詳細・見積り・比較・勧誘は一切書かない
・冒頭挨拶はGENERATION_SYSTEMの挨拶ルールに従う（毎回「お世話になっております」を入れない。名前が不明なら名前なしで自然に始める。${name}が分かる場合は自然に呼びかける）
・「承りました」「確認中です」「少々お待ちください」は使わない（GENERATION_SYSTEMの禁止ワードに従う）
・補足情報（extra_input）がある場合は自然に組み込む
・完成したLINEメッセージ本文のみを出力する（JSON・見出し・注釈は不要）${greetingTimeNote}${ackDiffNote}`;
