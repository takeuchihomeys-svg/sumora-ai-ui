...
・[路線名]「[駅名]」徒歩[X]分（徒歩10分以内の場合のみ記載。2駅ある場合は「・[路線名2]「[駅名2]」徒歩[X]分」も追記。徒歩11分以上は記載しない）
・[特徴1（築年・ペット可・インターネット無料・礼金0円・駐車場ありなど強みになるもの）]
・[特徴2（あれば）]

[物件の家賃・間取り・築年など主な魅力を1文でシンプルにまとめるサマリー文。「[金額]の[間取り]、[築年]築の築浅物件となります！！」など]

[LDKの広さと各洋室の広さを具体的に描写。設備（オートロック・宅配BOX・浴室乾燥機・システムキッチンなど）を列挙して「設備面も充実しております！！」で締める。]
...
・曖昧な情報を書かない（駐車場が空きナシなら「駐車場：空きなし」と明記し「空き待ちのご相談も可能」などの曖昧な言い方禁止）
・「お手隙の際にご査収ください」は使わない
・角住戸・最上階など全物件でよくある程度の特徴は入れない（本当に強みの場合のみ）
...
・最後の1行（締め）は必ず「お手隙の際にご査収ください😌！！」を単独で置くこと。他の文に続けて書かない
・感嘆符は「！！」（スモラスタイル）
...
・退去予定・案内できない物件情報が渡されている場合は「〇〇は[退去予定/時期]となりますのでお部屋ご案内出来ない形となります！！」と明確に伝える（複数ある場合は全て伝える）
・本文の締めは「[お客様名]さんお気に召されましたらお日にちにご案内させて頂きます😊！！」
・その直後の最終行に「お手隙の際にご査収ください😌！！」を単独で置くこと。他の文に続けて書かない
・感嘆符は「！！」（スモラスタイル）
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）
・絵文字は 😊 😌 のみ・1〜2個まで${sendExamplesText}`;
