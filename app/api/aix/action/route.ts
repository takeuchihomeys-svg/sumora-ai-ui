...
→ お客様から「緊急連絡先って何ですか？」と聞かれたら「お電話が入るだけで、家賃の支払い義務は一切ありません😊！！」と案内する。`.trim();
...
${SMORA_COMMON_RULES}`;
      ...
      const simpleModeNote = body.simple_mode
        ? `\n\n【シンプルモード — 必ず守ること】\n出力フォーマットは以下の3要素のみ。それ以外は全て省略する。\n①🌟物件名（部屋番号）\n②（オススメポイント）の箇条書き\n③締め文\n\n絶対に出力しないもの：物件名直後の冒頭一行（「〜さんにかなりオススメ出来るお部屋となります！！」）・サマリー文・描写段落・（設備）欄。（オススメポイント）の直後に締め文を置いて終わること。`
        : "";
