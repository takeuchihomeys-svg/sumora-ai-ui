...
// 学習済み差分ルール（analyze-diffsがスタッフ修正から学習したパターン）を対象stateから取得する汎用関数
// 「修正→学習→改善」ループの出口。各アクションのsystemプロンプト末尾に注入して使う
async function getDiffKnowledgeForState(states: string[]): Promise<string> {
  ...
  try {
    ...
    if (!data || data.length === 0) return "";
    // 使用追跡（fire-and-forget）
    const usedIds = (data as { id: string }[]).map(r => r.id).filter(Boolean);
    if (usedIds.length) {
      supabase.rpc("increment_knowledge_used_count", { p_ids: usedIds }).then(() => {}, () => {});
    }
    return "\n\n【🔴 過去の修正パターン（必ず守る）】\n" +
  ...
  "commission_tax": 仲介手数料消費税（なければ0）
      ...
      try {
        const [coverDiffNote, coverStarNote] = await Promise.all([
          getDiffKnowledgeForState(AIX_ACTION_TO_STATES.estimate_sheet),
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数・見積書の金額の繰り返しは禁止）${coverDiffNote}${coverStarNote}`;
  ...
  条件から主なポイント（エリア・家賃・間取り等）を自然に組み込む`
      ...
      const [sendDiffNote, sendStarNote] = await Promise.all([
        getDiffKnowledgeForState(AIX_ACTION_TO_STATES.property_send),
...
お手隙の際にご査収ください😌！！${sendExamplesText}`;
      ...
      message_text = await callClaude(sendSystem + sendDiffNote + sendStarNote, userParts.join(""));
    ...
    // ── 🔍 内覧へ！ ──────────────────────────────────────────────
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）＋☆成功返信パターンをプロンプト末尾に注入
      const [viewingDiffNote, viewingStarNote] = await Promise.all([
        getDiffKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite),
...
【お客様の呼び名 — 最重要ルール】
・会話履歴でスタッフが「〇〇さん」と呼んでいた名前を必ず使う（例：「ニアさん」「浜井さん」）
・会話全体を通じて使われてきた名前を途中で変えない（LINE表示名が1文字・英字のみの場合は会話履歴の呼び名を優先）
・以下の締め文の[お客様名]は必ず会話履歴から読み取った実際の呼び名を入れる

【状況別パターン — 会話の流れを読んで最適なものを1つ選ぶ】

▼ パターンA：内覧希望が来た・日程提案あり（calendar_info がある場合に使う）
①受け止め：「かしこまりました！！」
...
▼ パターンB：内覧希望が来た・日程なし（後日カレンダー確認）
①受け止め：「かしこまりました！！」
②内覧提案：「[お客様名]ご都合よろしいお日にちにお部屋ご案内させて頂きます！！」

▼ パターンC：物件送付後・物件を気に入った後の内覧誘導
①共感（任意）：「お気に召されて嬉しいです！！」など文脈に合わせた一言
②内覧提案：「[お客様名]ご都合よろしいお日にちにお部屋ご案内させて頂きます！！」
③アピール（任意）：「お部屋ご内覧頂きますとより魅力伝わる物件ですのでご案内させて頂きます😊！！」

▼ パターンD：退去後に内覧可能になった通知（退去済みフラグあり）
①報告：「[物件名]退去しましたのでお部屋ご内覧可能です😊！！」
②内覧提案：「[お客様名]ご都合よろしいお日にちにお部屋ご案内させて頂きます！！」
③締め：「お気軽にお申し付けください😌！！」

▼ パターンE：退去予定日あり・○日以降ご案内可能（vacancy_status = scheduled）
①物件状況：「[物件名]現在募集中となります！！」
②退去予定：「[退去予定日]退去予定のお部屋で[翌日]以降でお部屋ご案内可能です！！」
③内覧提案：「[お客様名][翌日]以降のご都合よろしいお日にちにご案内させて頂きます😊！！」
...
【絶対禁止】
・「いかがでしょうか？」など「？」のみで終わる締め → 必ず末尾は「！！」
・パターンをひとつの文にまとめない（改行して構成を守る）
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
      ...
      message_text = await callClaude(system + viewingDiffNote + viewingStarNote, `${name}への内覧お誘いメッセージ。${propNamePart}${vacancyPart}${calendarPart}${templateStructureNote}${recentHistory}`);
    ...
    // ── ✋ 申込へ！ ──────────────────────────────────────────────
      ...
      const [appDiffNote, appStarNote] = await Promise.all([
        getDiffKnowledgeForState(AIX_ACTION_TO_STATES.application_push),
...
${SMORA_COMMON_RULES}`;
        ...
        message_text = await callClaude(confirmSystem + appDiffNote + appStarNote, `${name}への申込確定メッセージ。${property_name ? `物件名:${property_name}。` : ""}${recentHistory}`);
...
${examplesText}`;
      ...
      message_text = await callClaude(system + appDiffNote + appStarNote, userMsg);
...
・完成したメッセージのみ出力`;
      ...
      const moveInDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result);
      ...
      message_text = await callClaudeVision(moveInSystem + moveInDiffNote, content);
...
・完成したLINEメッセージのみ出力（候補複数・前置きは禁止）`;
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const mgmtDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result);
...
${templateText}`;
        ...
        // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入（自由生成パスのみ）
        const checkDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result);
        ...
        if (image_url || estimate_image_url) {
          ...
          message_text = await callClaudeVision(checkSystem + checkDiffNote, content);
        } else {
          message_text = await callClaude(checkSystem + checkDiffNote, userText);
        }
      }
...
${formItems}`;
      ...
      try {
        const [hearingDiffNote, hearingStarNote] = await Promise.all([
          getDiffKnowledgeForState([...AIX_ACTION_TO_STATES.condition_hearing, "first_reply"]),
...
・50〜100文字程度・2〜3行まで
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）
・条件項目の箇条書き自体はこのメッセージに含めない（フォームは別送するため）${hearingDiffNote}${hearingStarNote}`;
...
・3行以外の追加は一切しない。解説・絵文字・補足は不要`;
        ...
        // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
        const beforeDiffNote = await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing);
        ...
        const afterDiffNote = AFTER_FIXED_TYPES.includes(after_type ?? "")
          ? ""
          : await getDiffKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing);
...
【スモラLINE営業ルール（必ず守る）】
${SMORA_COMMON_RULES}`;
          message_text = await callClaude(sys + afterDiffNote, `確認事項: ${freeword}${recentHistory}`);
...
【スモラLINE営業ルール（必ず守る）】
${SMORA_COMMON_RULES}`;
          message_text = await callClaude(sys + afterDiffNote, `条件: ${freeword}${recentHistory}`);
...
【スモラLINE営業ルール（必ず守る）】
${SMORA_COMMON_RULES}`;
          message_text = await callClaude(sys + afterDiffNote, `変更条件: ${freeword}${recentHistory}`);
...
・「！！」を文末に使う・3行のみ出力`;
          message_text = await callClaude(system + afterDiffNote, `${name}への内覧後挨拶を生成してください。${recentHistory}`);
        }
      }
      ...
      const [meetingDiffNote, meetingStarNote] = await Promise.all([
        getDiffKnowledgeForState(AIX_ACTION_TO_STATES.meeting_place),
...
${SMORA_COMMON_RULES}`;
      ...
      message_text = await callClaude(system + meetingDiffNote + meetingStarNote, `会話履歴から待ち合わせ時間を読み取り、メッセージを生成してください。${recentHistory}`);
