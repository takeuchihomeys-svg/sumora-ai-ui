...
// 学習済みナレッジを対象stateから取得する汎用関数（差分学習ルール＋非差分のpattern/principle/phrase）
// 「修正→学習→改善」ループの出口。各アクションのsystemプロンプト末尾に注入して使う
// getPropertyKnowledge()の汎用版。全フリー生成アクションで使用。
async function getKnowledgeForState(states: string[]): Promise<string> {
  if (!states || states.length === 0) return "";
  try {
    const [{ data: diffLearned }, { data: otherKnowledge }] = await Promise.all([
  ...
  "commission_tax": 仲介手数料消費税（なければ0）
      ...
      try {
        const [coverDiffNote, coverStarNote] = await Promise.all([
          getKnowledgeForState(AIX_ACTION_TO_STATES.estimate_sheet),
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数・見積書の金額の繰り返しは禁止）${greetingTimeNote}${coverDiffNote}${coverStarNote}`;
  ...
  条件から主なポイント（エリア・家賃・間取り等）を自然に組み込む`
      ...
      const [sendDiffNote, sendStarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.property_send),
...
お手隙の際にご査収ください😌！！${sendExamplesText}`;
    ...
    // ── 🔍 内覧へ！ ──────────────────────────────────────────────
      ...
      if (rescheduleMode) {
        const rescheduleDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite);
...
・2〜4行程度・完成したLINEメッセージのみ出力${greetingTimeNote}${rescheduleDiffNote}`;
      ...
      const [viewingDiffNote, viewingStarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.viewing_invite),
...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
    ...
    // ── ✋ 申込へ！ ──────────────────────────────────────────────
      ...
      const [appDiffNote, appStarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.application_push),
...
${SMORA_COMMON_RULES}`;
      ...
      const moveInDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result);
...
・完成したLINEメッセージのみ出力（候補複数・前置きは禁止）`;
      ...
      // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
      const mgmtDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result);
...
${templateText}`;
        ...
        const [checkDiffNote, checkStarNote] = await Promise.all([
          getKnowledgeForState(AIX_ACTION_TO_STATES.property_check_result),
...
${formItems}`;
      ...
      try {
        const [hearingDiffNote, hearingStarNote] = await Promise.all([
          getKnowledgeForState([...AIX_ACTION_TO_STATES.condition_hearing, "first_reply"]),
...
・3行以外の追加は一切しない。解説・絵文字・補足は不要`;
        ...
        // 学習済み差分ルール（スタッフ修正から学習したパターン）をプロンプト末尾に注入
        const beforeDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing);
        ...
        const afterDiffNote = AFTER_FIXED_TYPES.includes(after_type ?? "")
          ? ""
          : await getKnowledgeForState(AIX_ACTION_TO_STATES.greeting_viewing);
...
・「！！」を文末に使う・3行のみ出力`;
      ...
      const [meetingDiffNote, meetingStarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.meeting_place),
...
${SMORA_COMMON_RULES}`;
    ...
    // ── ✅ 確認します ──────────────────────────────────────────────
      ...
      const ackDiffNote = await getKnowledgeForState(AIX_ACTION_TO_STATES.acknowledge_check);
...
・余計な説明や長文は不要。完成したLINEメッセージのみ出力${greetingTimeNote}${ackDiffNote}`;
    ...
    // ── 📣 追客する ──────────────────────────────────────────────
      ...
      const [followupDiffNote, followupStarNote] = await Promise.all([
        getKnowledgeForState(AIX_ACTION_TO_STATES.followup_revive),
