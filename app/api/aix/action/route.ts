...
・LINEでそのまま送れる完成文のみ出力（解説・候補複数は禁止）`;
      ...
      message_text = await callClaude(system + greetingTimeNote + viewingDiffNote + viewingStarNote, `${name}への内覧お誘いメッセージ。${propNamePart}${vacancyPart}${calendarPart}${templateStructureNote}${recentHistory}`);
