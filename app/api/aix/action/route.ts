...
【あなたのタスク：ベースメッセージを会話に合わせて仕上げる】
以下の「ベースメッセージ」は既に高品質に生成されたメインメッセージです。
このメッセージをベースとして、直近の会話で出てきたお客様の要望・状況に自然に対応する文を作ってください。

【ベースメッセージ（この内容・構成・トーンを基本とすること）】
${baseMessage}

【やること】
・ベースメッセージの内容はそのまま活かす（ゼロから書き直さない）
・冒頭1〜2文を、お客様の直近メッセージへの直接的な応答に調整する
・お客様が会話で出していた要望・懸念・条件（日程・物件名・質問など）を自然に反映する
・ベースメッセージがカバーしていないお客様の追加要望がある場合は、末尾に1〜2文で自然に追加する
  例: 物件確認の文に加えて「引き続き条件に合うお部屋をピックアップさせて頂きます！！」を追加するなど
...
{"message":"〜（調整後のLINEメッセージ全文・改行は\\nで）"}`;
  ...
  const raw = await callClaude(
    system + diffNote,
    `${conversationHistory}\n\n上記の会話を深く読み取り、ベースメッセージをお客様の要望に合わせた最終メッセージを出力してください。`,
    actionLabel
  );
  try {
