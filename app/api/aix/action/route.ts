...
・節約金額・費用比較の文言（「〇〇円節約出来ます」「一般的な不動産業者より〜」等）はすでに見積書本文に別途表示済みのため、カバーレターには絶対に含めないこと。${greetingTimeNote}${coverDiffNote}${coverStarNote}`;
    ...
    // ── 📤 物件ピックアップした ──────────────────────────────────────────────
      ...
      // 内覧提案を省略トグル（UI側で「内覧提案を省略」選択時に false が届く）
      const skipViewingInvite = body.include_viewing_invite === false;
...
LICC系: ジェイリース、JID、全国保証、アート・プランニング、青山ライフデザイン、保証ベース
独立系（最も審査緩い）: 日本セーフティー、エルズサポート、Casa、フォーシーズンズ、ルームバンク、全保連、いえらぶ保証、スマートタカミ、イントラスト、レジデンシャルパートナーズ、日本トラストコーポレーション、株式会社日本トラストコーポレーション`;
