...
▼ rule の文体指示（重要）：全シナリオ共通の禁止・必須ルールなら「必ず〜すること」「〜してはいけない」「〜は禁止」を使うこと。場面依存のパターンならそれ以外の表現を使うこと。
...
const POLICY_KEYWORDS = /禁止|絶対|必ず|してはいけない|しないこと|してはならない|使用禁止|送ってはいけない|言ってはいけない|を徹底|に徹する|することが重要|が重要です|を心がける|を心がけ|すべきです|べきです|が望ましい|を避ける|を避け|NG表現|推奨しない/;
