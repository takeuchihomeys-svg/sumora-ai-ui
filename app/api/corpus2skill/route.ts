...
- 🔴の例で「AI案にあってスタッフが削除した文言」がある場合は、それを**禁止パターン**として必ず抽出すること。
  title は "[警告] ..." で始め、content は「『〜』という文言は顧客が自ら△△と言った場合のみ使う。通常の○○（例：内覧希望のみの顧客）には絶対に使わない」のように、使ってよい条件と絶対に使わない場面を明記した禁止条件付きスキルとして書く（特定顧客の特殊要望由来のフレーズが汎用化して別顧客に誤出力されるのを防ぐため）
