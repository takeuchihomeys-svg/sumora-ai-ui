...
function buildGenerationMessages(
  ...
  const greetingNote = alreadyGreeted
    ...
    : (state === "first_reply" && isFirstEverReply)
      ? `\n【⏰ 初回対応ルール・最優先】これはお客様への【はじめての返信】。必ず「〇〇さん、はじめまして😊！！この度ご連絡頂きありがとうございます！！お部屋探しを担当させて頂きます鈴木と申します！！」の形式で始める。「お世話になっております」「夜分遅くに失礼致します」は絶対禁止。${jstHour >= 18 ? "18時以降のため「本日中に」は禁止。「明日一番で」を使うこと。" : ""}`
      : jstHour >= 21
