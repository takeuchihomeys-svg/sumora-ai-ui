...
function createAnalysisModel() {
  return new ChatAnthropic({
    ...
    betas: ["prompt-caching-2024-07-31", "extended-cache-ttl-2025-02-19"],
  });
}
...
・【📌 見積書・初期費用への言及は「お客様が質問している場合のみ」（最優先ゲート）】お客様の最新メッセージが初期費用・見積書・費用について質問・依頼している場合のみ、見積書作成宣言を使ってよい。お客様が「前向きに検討しています」「決まり次第ご連絡します」「かしこまりました」「よろしくお願いします」「ありがとうございます」等、費用・見積書に触れていない場合は、見積書・初期費用の説明・作成宣言・割引の言及を一切含めず、承諾とサポート姿勢のみで返信を締めること。過去の会話で初期費用の話題があったとしても、現在のメッセージが費用と無関係であれば絶対に蒸し返さない。
