...
const STATE_FALLBACK_DIRECTION: Record<string, string> = {
  ...
  proposing:
    ...
    // 2026-09-23 竹内（あっぴ事例）「家賃交渉は基本できないものだからいれない。この言い回しいれないようにする」:
    //   旧 ⑤「家賃・条件の交渉」が、こちらから家賃交渉を宣言する言語的経路そのものだった。
    //   実送信（365日・12,417通）で家賃・賃料の値下げ交渉の予告は0通。交渉の宣言が実在するのは
    //   礼金・敷金・管理費・初期費用・仲介手数料・支払い方法だけで、その15件は全件お客様が費用の相談をした後。
    //   選択肢は残す（列挙で示す・「〜の1文を添えろ」型にしない）が、家賃を対象から外し条件を付ける
...
※例外: お客様が【新しい物件】（URL・物件画像・まだ確認していない物件名）を新たに挙げた場合、その物件についての確認宣言は正当。上記禁止は既に実行済みの物件・依頼にのみ適用する`;
  ...
  //   （「させて頂きます」201回消され／「させていただきます」262回足される）。
...
読み取れない場合は「読み取れませんでした」のみ返す。余計な説明不要。` },
  ...
  try {
    ...
    //   お客様が新しい依頼ではなく謝っている時は、冒頭を受け止めの言葉にする（材料＋出口の両方）
    const apologyVerdict = resolveApologyOnly(intentMessage);
    if (apologyVerdict.apology) console.info("[apology-ack] 謝罪を受け止める場面", JSON.stringify({ conversationId, reason: apologyVerdict.reason }));
    ...
    const gratitudeActionHint: string = (() => {
      ...
      // 2026-09-16 竹内（YUYA 事例）: 内覧当日の送り出し（「お気をつけてお越し下さい」）の後のお礼に
      //   「明日以降も気になる点等…」と書いていた。送り出しの後は日付に触れず現地で待つ形で受ける
      //   （この場面はブレインが AIX【挨拶（内覧後）】をセットして自動の下書きを止める。手動生成の時の保険）
      if (/お気をつけて|お越し(?:ください|下さい)/.test(s))
    ...
    const tpoNoteForLLM: string | null = (() => {
      ...
      if (phaseGuideKey === "proposing" && !a) {
        ...
        return "商談継続中の汎用返答。顧客の質問・要望を1文で受け止め、具体的な行動宣言を1つだけ添えて100〜150字で返す（次のいずれか1つ: ①内覧のご案内提案「よろしければ〇〇さんご都合よろしいお日にちにお部屋ご案内させて頂きます😌！！」＝具体的な候補日時は書かない（AIX【内覧日調整】専用）②募集状況の確認 ③御見積書の作成・送付 ④ご条件に合うお部屋のピックアップ ⑤費用の交渉（礼金・敷金・管理費・初期費用・仲介手数料のみ。お客様が費用について頼んだ・聞いた時だけ選ぶ。家賃・賃料の値下げ交渉はこちらから宣言しない））。抽象的な「確認します」禁止";
      }
      return null;
    })();
    ...
    const brainGuidanceNote = (() => {
      ...
      if (closingGatedByStance) {
        ...
        //   ただし PEAK/STRONG の「今回の返信で申込期限・書類リスト・CTA」は、ブレインが今回の発言を見て決める一手なので発動しない
        lines.push(`- 🌡️ 購買シグナル（前回の分析までの蓄積・参考）: ${String(brainMeta.purchase_signal_level).toUpperCase()} — 会話全体の熱量としてトーンにだけ反映する。今回のお客様の発言はまだ分析されていないため、申込期限の明示・申込書類の案内・完全クロージングは今回は入れず、最新メッセージへの応答を優先すること`);
                  ...
                  try {
                    ...
                    const feedback = [
                      ...
                      ...(pairContext.rule
                        ...
                        // 2026-09-23 点検（あっぴ事例の続き）: ここだけ語順が「条件・家賃の交渉」だったため 9/23 の一括置換から漏れていた。
                        //   作り直しの指示は**プロンプトの最後**に付く＝最も強い位置で、同じ呼び出しの中の
                        //   「言及禁止: 家賃交渉」（ブレインの avoid_topics）と正面から矛盾していた。他の3か所と同じ文言に揃える
                        : [`- 【WE DO の選択肢】次のいずれか1つだけを文脈から選ぶ（複数並べない）: ①内覧のご案内提案（「${fillPairPlaceholders("{viewingOffer}", pairContext)}」・具体的な候補日時は書かない）②募集状況の確認 ③御見積書の作成・送付 ④ご条件に合うお部屋のピックアップ ⑤費用の交渉（礼金・敷金・管理費・初期費用・仲介手数料のみ。お客様が費用について頼んだ・聞いた時だけ選ぶ。家賃・賃料の値下げ交渉はこちらから宣言しない）`]),
