...
LINEの会話・条件メモ・物件検索サイトの設定画面など、どのような形式でも対応します。

【物件検索サイト（リアプロ・itandi・REINS等）の駅・沿線選択画面の場合 — 最重要】
- 駅名や沿線名がボタン形式で並んでいる画面の場合、「選択中・ハイライト中・チェック済み」の項目だけを読み取る
- 選択中の目印: 背景色が濃い・オレンジ/青/緑などに塗りつぶされている・チェックマークがついている・枠が強調されている
- 全ての駅名を列挙するのではなく、明らかに選択されている項目のみを「エリア/駅」として抽出する
- 右サイドパネル（設定中の沿線・設定中の駅）がある場合はそちらを優先して読む
- 家賃設定（上限・下限）の入力欄がある場合は読み取る

【その他の画像の場合】
- LINEの会話: お客さんが送ってきた条件のメッセージを読む
- 手書きメモ・テキスト条件表: そのまま読む

【読み取り対象】エリア・駅名・間取り・家賃・駅徒歩・広さ・築年数・入居時期・初期費用・こだわり・NG条件など
