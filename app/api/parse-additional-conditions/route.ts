...
- 読み取れない場合のみ「条件が読み取れませんでした」と返す`,
    ...
    const extractedText = extractData.content?.find((b): b is typeof b & { text: string } => b.type === "text")?.text?.trim() ?? "";
    ...
    const raw = parseData.content?.find((b): b is typeof b & { text: string } => b.type === "text")?.text ?? "";
