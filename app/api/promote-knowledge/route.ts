...
export async function POST(req: Request) {
  // Vercel本番環境ガード: 読み取り専用FSのため fs.writeFile / tsc / git が動作しない
  if (process.env.VERCEL) {
    return NextResponse.json(
      { ok: false, error: "This endpoint is only available in local development" },
      { status: 400 }
    );
  }
...
JSONのみ返す：{"additions":[{"title":"ルール名","formatted":"フォーマット済みテキスト（改行含む）"}]}`);
  ...
  // git commit & push
  // コマンドインジェクション対策: シェル文字列連結をやめ、execFile の引数配列で渡す
  // （ruleNames はAI応答由来のため " や $() 等が含まれても安全に扱う）
  const ruleNames = report.flatMap((r) => r.rules).join("・");
  const commitMessage = `auto: PHASE_GUIDE自動昇格（${totalAdded}件）${ruleNames ? " - " + ruleNames : ""}`;
  try {
    const cwd = process.cwd();
    await execFileAsync("git", ["add", "app/lib/line-reply-prompts.ts"], { cwd });
