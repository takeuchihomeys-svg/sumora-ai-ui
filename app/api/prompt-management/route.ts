...
・既に見積書を送った後に「ありがとうございます」など反応が来た場合 → 見積書は出さない。申込へも進めない（内覧のご案内が中心・次の一手はお客様の反応で判断）`,
  },
...
・見積書を要求された場合 → 「見積書送る」を使う`,
  ...
  aix_logic_property_check: {
    label: "物件確認した／確認した（条件・交渉）",
    group: "aix_logic",
    content: `【AIX「物件確認した／確認した（条件・交渉）」の発動条件】

※ UIでは「物件確認した（募集状況）」と「確認した（条件・交渉）」の2ボタンだが、
　どちらも同じ property_check_result（check_patternで切替）。
　空室・退去日・入居可能日・保証会社・初期費用交渉・駐車場・ペット可否など、
　管理会社・代表・オーナー・近隣月極への確認結果を報告する。
...
・お客様が物件画像・動画を送ってきた
・「空室確認」「募集状況確認」を依頼された
・「保証会社はどこ」「保証料は」「ペット飼えますか」「駐車場ありますか」「礼金下げられますか」など管理会社確認が必要な物件固有の質問 → 「確認した（条件・交渉）」で結果報告

■ 発動しないケース
・スタッフが物件URLを送った場合（スタッフ側の送信）
・既に空室確認済みで回答した後
・保証会社の役割や審査の流れなど一般論の質問 → 通常返信AIで即答`,
  },
