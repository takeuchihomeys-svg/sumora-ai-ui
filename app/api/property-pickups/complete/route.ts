...
import { completeAuthOk, completeToastJa } from "@/app/lib/pickup-complete";
...
const TRIGGERS = new Set(["viewed", "sent", "manual"]);
...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => ({})) as { property_customer_id?: unknown; brain?: unknown; mode?: unknown; trigger?: unknown; requested_by?: unknown };
  ...
  const trigger = typeof body.trigger === "string" && TRIGGERS.has(body.trigger) ? body.trigger : null;
  ...
  const claim = await claimCompleteGroup(pcid, { trigger, mode, requestedBy });
  ...
  return NextResponse.json({
    ...
    toast: completeToastJa({ claimed: claim.claimedIds.length, sites: claim.sites, already: claim.already }),
  }, { headers: CORS });
}
