...
━━━━━━━━━━━━━━━━━━━━
このテンプレートはAIXが生成した「確定済みの文」です。以下のルールを全てのルールより優先して守ること。

【置き換えてよいもの — これだけ】
・「アカウント名」「〇〇さん」「□□さん」「名前プレースホルダー」→「${customerName || "〇〇"}さん」に置き換える
・明示的に「〇〇」「○○」と書かれたプレースホルダーのみ、会話・条件に情報があれば埋める
・「様」→「さん」の変換

【絶対に変えてはいけないもの】
・物件名・間取り・専有面積・帖数・築年数・家賃・敷金礼金・設備の記述 → 一字一句そのまま
・文章の構成・段落の順番・語順 → 変えない
・訴求ポイント（「敷金礼金0円」「南向き」等の強み文）→ 変えない
・「やること②（固有名詞置き換え）」「やること③（訴求書き換え）」は【AIXテンプレートモードでは禁止】
・会話履歴・お客様条件・AIXメッセージから物件情報を書き換えるのは禁止
