...
const AIX_TEMPLATES: Record<AixActionType, { rules: string[]; template: string }> = {
  ...
  cost_explain: {
    rules: [
      "費用の安さを不審に思われた・聞かれた時に使う",
      "貸主からの報酬と還元額を入力 → 仕組みと具体額を1通で生成（AI不使用）",
      "💡 仕組みを説明（金額なし）を選べば、報酬額を入れずに仕組みだけの1通が作れる",
      "仲介手数料の書き方はアカウントで変わる: スモラ＝一律2,980円／イエヤス・ギガ＝0円",
      "貸主から手数料が無いお部屋は「割引出来ないが一般的な不動産業者より〇〇円お得」",
    ],
    template: "【スモラ】ご質問ありがとうございます😊！！\n弊社は仲介手数料2,980円のみでご案内させて頂いております！！オーナー様からの広告料をお客様に還元させて頂いている仕組みのため、初期費用を一般的な不動産業者様よりお安くご提案出来ております！！\n\n他社様との金額差はこの還元の有無によるものですので、ご安心ください😊！！\n\nこちらの物件は貸主から[家賃1ヶ月分の]手数料を弊社不動産仲介会社は頂く事が出来ます！！\n[報酬]円を貸主から頂き、そこから[還元額]円を[お客様名]さんの初期費用に還元させて頂きますので、弊社としましても利益残りますのでご安心頂けますと幸いです！！\n\n【イエヤス・ギガ】冒頭が「弊社は仲介手数料0円となります！！」に変わります",
  },
  cost_breakdown: {
...
> = {
  ...
  cost_explain: {
    ...
    requiresImage: false,
    imageLabel: "",
    description: "費用の安さを不審に思われた・聞かれた時に、仕組み（広告料の還元／スモラは仲介手数料2,980円・イエヤスとギガは0円）とこの物件の具体額を1通で説明します。金額が要らない時は「仕組みを説明（金額なし）」を選んでください。",
  },
  cost_breakdown: {
    title: "初期費用について",
    emoji: "🧾",
...
export default function AixModal({
  ...
  // M1: 「物件確認した」で確認した物件名と各物件の状態。
  // onAfterSend 経由で log-aix-usage の property_names / prop_statuses に記録され、
  // brain-core が【物件別空き状況（確定事実）】としてプロンプトに注入する。
    ...
    try {
      ...
      if (actionType === "property_recommendation") {
        ...
        if (checkPattern === "available") {
          ...
          // M1: 募集終了物件を確定事実として永続化（brain の「再確認宣言禁止」判定ソース）
          if (checkUnavailablePropName.trim()) {
            lastCheckPropNamesRef.current = [checkUnavailablePropName.trim()];
            lastCheckPropStatusesRef.current = ["unavailable"];
          }
    ...
    ? (checkPattern === "move_in_date" ? !!moveInImageFile
      ...
      : isMgmtCheck ? (
          checkPattern === "mgmt_initial_cost"
            ...
            : checkPattern === "mgmt_availability"
            ? !!mgmtAvailabilityStatus
            : !!inputText.trim()
        )
      : !!checkPattern)
        ...
        <div className="max-h-[75vh] overflow-y-auto p-5">
          ...
          {actionType === "property_recommendation" ? (
            ...
            <div className="mb-4 flex flex-col gap-3">
              ...
              {/* 募集状況確認専用: 2択（募集している / 募集終了した） */}
              {checkPattern === "mgmt_availability" && (
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="mb-2 text-xs font-bold text-[#54656f]">募集状況 <span className="text-red-400">*</span></p>
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => { setMgmtAvailabilityStatus("available"); setPreview(""); }}
                        className={`flex-1 rounded-xl border-2 py-2.5 text-xs font-bold transition ${
                          mgmtAvailabilityStatus === "available"
                            ? "border-green-500 bg-green-50 text-green-700"
                            : "border-[#e9edef] bg-[#f8f9fa] text-[#9CA3AF]"
                        }`}
                      >募集している</button>
                      <button
                        onClick={() => { setMgmtAvailabilityStatus("ended"); setPreview(""); }}
                        className={`flex-1 rounded-xl border-2 py-2.5 text-xs font-bold transition ${
                          mgmtAvailabilityStatus === "ended"
                  ...
                  <div>
                    ...
                    <textarea
                      value={inputText}
                      onChange={(e) => { setInputText(e.target.value); setPreview(""); }}
                      placeholder="例：申込はまだ入っていない、今週中なら先行申込可 等"
                      rows={2}
                      className="w-full resize-none rounded-xl border border-[#d1d7db] px-3 py-2.5 text-sm text-[#111b21] outline-none focus:border-[#2196F3] placeholder:text-[#8696a0]"
                    />
                  </div>
