...
export default function BottomNav({ unreadCount = 0, hidden = false }: Props) {
  ...
  return (
    ...
    >
      {/* メッセージ */}
      <Link href="/" className="flex flex-1 items-center justify-center py-1">
        <span className="relative flex items-center">
          ...
          {unreadCount > 0 && (
            <span
              className="absolute -right-0.5 -top-1.5 flex h-[16px] min-w-[16px] items-center justify-center rounded-full px-0.5 text-[8px] font-bold text-white leading-none"
      ...
      {/* カレンダー */}
      <Link href="/calendar" className="flex flex-1 items-center justify-center py-1">
        <span
          className="flex items-center justify-center rounded-full transition-all duration-200"
          style={{
            width: 44,
