...
it("★ 金額ブロックと見出しが同時にあっても本文は残る（本番の混入の全部入り）", () => {
  const t = "【お客様に送る文】\nYUMAさんご連絡頂きありがとうございます😊！！\n\n**【プレサンス阿倍野松崎805号室】**\n\n初期費用さらに\n🌟68,000円割引させて頂き\n初期費用：152,000円\n\n※ご入居日によって日割家賃が発生致します。\n\n最大限割引しました初期費用の御見積書となります！！ご確認よろしくお願いします！！";
  const { text } = sanitizeCoverLetter(t, "YUMAさん");
  expect(text).toBe("YUMAさんご連絡頂きありがとうございます😊！！\n\n最大限割引しました初期費用の御見積書となります！！ご確認よろしくお願いします！！");
});

it("★ 金額ブロックを落とした結果、見積書の話が1つも残らなければ送らない", () => {
  // 本番で出た実物（この文には「見積」が1度も出てこない）
  const t = "YUMAさんご連絡頂きありがとうございます😊！！\n\n**【プレサンス阿倍野松崎805号室】**\n\n初期費用さらに\n🌟68,000円割引させて頂き\n初期費用：152,000円\n\nご確認よろしくお願いします！！";
  expect(sanitizeCoverLetter(t, "YUMAさん").text).toBe("");
});


console.log("\n── ★ 受け入れ条件: 見積書の話をしていない2通目は送らない（3回転目）──");

it("★「お申込み情報を確かに受け取りました」だけの文は送らない（見積の語が無い）", () => {
  for (const t of [
    "YUMAさんお世話になっております😊\nお申込み情報を確かに受け取りました！！",
    "お申込み情報拝受いたしました。今後のお手続きについてご連絡させていただきます。",
    "内容をこちらで確認することができない状態でございます。",
  ]) {
    const r = sanitizeCoverLetter(t, "YUMAさん");
    if (r.text !== "") throw new Error(`送ってしまう: ${t}`);
  }
});

it("★ 実送信のカバーレター859通は全部「見積」を含む＝1通も落ちない", () => {
  for (const t of [
    "YUMAさんお待たせ致しました！！\nスプランディッド大阪EAST 603号室最大限割引しました初期費用の御見積書となります！！\nお手隙の際にご査収ください😌！！",
    "ゆーたさん確認させていただきました！！\n初期費用御見積書同封させて頂きました！！",
    "こちらのお部屋スモ割が適用出来ないお部屋となっており、お送りさせていただきました初期費用お見積書がご案内できる最安値のお見積書となります😌！！",
    "大野さんお世話になっております！！\n\n【エスティメゾン南堀江 1103号室】\n\n最大限割引した初期費用のお見積書作成しお送りさせて頂きます😊！！\n\nお手隙の際にご査収ください！！",
  ]) {
    const r = sanitizeCoverLetter(t, t.slice(0, 3));
    if (!r.text) throw new Error(`落としてしまった: ${t.slice(0, 40)}`);
  }
});

console.log("\n── ★ プロンプト丸写しの【…】（4回転目・実送信12,016通で0通）──");

it("★ プロンプトの節名【タスク】【初期費用内訳】を外して本文は残す", () => {
  expect(sanitizeCoverLetter("【タスク】初期費用の見積書を送付する際の添付メッセージを1つだけ作成してください", "YUMAさん").text)
    .toBe("初期費用の見積書を送付する際の添付メッセージを1つだけ作成してください");
  expect(sanitizeCoverLetter("【初期費用内訳】記載なし（見積書に金額を書かない）", "YUMAさん").text)
    .toBe("記載なし（見積書に金額を書かない）");
});

it("★ 実送信に本当にある【…】は1つも落とさない（387種類・上位は記入欄とフォーム項目）", () => {
  const legit = [
    "【ハイツカトレア B 202号室】\n最大限割引した初期費用のお見積書となります！！",
    "【お申込者様記入欄】\n御見積書と一緒にお送りします",
    "【緊急連絡先欄】\n御見積書に添えてお送りします",
    "【同居人記入欄】\nお見積書と併せてご確認ください",
    "（ご希望のお部屋探しご条件）\n④【希望築年数】\n⑦【初期費用の限度額】⇒\n①【ご入居の時期】⇒\nお見積書もお送りします",
  ...
  for (const t of legit) {
    const r = sanitizeCoverLetter(t, "YUMAさん");
    if (r.removed.length) throw new Error(`落としてしまった: ${r.removed.join(" / ")}`);
  }
});
