...
### 質問文の生成フォーマット（この2形式のみ生成する）

【昇格承認型】(ルールが昇格基準を満たした場合)
✅「ルールタイトル（60字以内）」
[フェーズ] correct:X回 / apply:X回 / 外れ率X%。confirmedに昇格してOKですか？
※ルール: ルール内容の冒頭120字

【適用場面不明確型】(autoJudgeがquestionを返した場合)
❓「ルールタイトル（60字以内）」

AI: 「AIが出した核心部分80字（\nを' 'に変換）」→ スタッフ: 「スタッフが修正した核心部分80字（\nを' 'に変換）」

[フェーズ] このルール、使う場面と使わない場面を「○○のとき〜する / △△のときは除外」の形で定義してください。
※確認点: 不明確な点の要約（120字以内）

### 質問生成時の絶対禁止事項
- ①②③ の選択肢列挙を使わない（番号だけで終わる回答を誘発するため）
- 「なければ特になし」「問題なければスキップ」等の逃げ道を作らない
- AI/スタッフの文面を400字まるごと引用しない（核心部分80字のみ）
- ルール内容を300字まるごと引用しない（120字以内に圧縮）
- 「どんな場面で使いますか？」という開放型の質問をしない（if/then形式を求める）`;
