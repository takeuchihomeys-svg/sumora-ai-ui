...
export function buildAixChainNote(firstMessage: string | null | undefined): string {
  ...
  // 2026-09-20 竹内「YUMA でテストして確認／実際の成約データや直近の文のようになっているか」:
  //   YUMA で8通生成したら **申込の誘導が100%**（実送信8.8%・成約8.8%）になり、
  //   「他のお客様からお申込みが入る可能性もございます」という希少性の煽りまで出た。
  //   原因は system プロンプトの骨格が**1通目用**だったこと:
  //     「1段落目: 今回何をお送りしたかの宣言／2段落目: 設備・間取りの訴求／3段落目: 立地／
  //      4段落目: 費用／**5段落目: CTA（内覧誘導または申込誘導）**」＝ 物件を送る本体の5段落構成。
  //   2通目にこれを当てると、毎回「お気に召されましたらお申込みで…」で終わる。
  //   設計知見「同じ事実について書くなと書けを別の場所から渡さない」→ **骨格をここで明示的に上書きする**。
  L.push("");
  L.push("【⚠ ここまでの指示のうち、次の4つは1通目用なので今回は使わない（この段落が最優先）】");
  L.push("　①「文章構造の原則」の5段落構成（1段落目=今回何を送ったかの宣言／2=設備／3=立地／4=費用／5=CTA）");
  L.push("　②「この種別の書き方」の CTA（「お気に召されましたらお申込みしお部屋抑えさせて頂きます」等）");
  L.push("　③「CTA強度の上書き」");
  ...
  //   **付けるな**ではなく**毎回は付けるな**。お客様が前向きな時は付ける、が実送信の形。
  L.push("CTA（内覧・申込の誘い）は**毎回は付けない／付けてはいけない訳でもない**。");
  L.push("実送信の2通目では 申込の誘い8.8%・内覧の誘い6.1%＝**約7通に1通**は付いている。");
  L.push("お客様が前向きな反応（気に入った・内覧したい・申込みたい）を見せている時は付けてよい。");
  L.push("そうでない時は見立てを伝えて終わってよい（無理に誘わない）。");
  // 2026-09-20 竹内「残る差もテストして改善する」:
  //   YUMA で測ると 名前呼びかけ100%（実送信の2通目35.1%）・条件の復唱0%（30.7%）・物件名13%（36.0%）。
  //   呼びかけは1通目用の「実名のみ」指示を「毎回付ける」と読んでいた。
  //   条件・物件名は「並べ直さない」を「触れてはいけない」と読んで消えすぎた。
  L.push("");
  L.push("【2通目で書いてよいもの・毎回は要らないもの（実送信の2通目の割合）】");
  L.push("・呼びかけ「〇〇さん」… 35%。**毎回は付けない**（付ける時は実名のみ・伏せ字は禁止）");
  L.push("・物件名・号室 … 36%。**1件に絞って推す時は書いてよい**（ただし1通目に出ている物件だけ）");
