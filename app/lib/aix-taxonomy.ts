...
export const AIX_BUTTON_LABELS: Record<string, string> = {
  ...
  // 2026-09-15 竹内（YUYA 事例）: 物件ごとの保証会社名と種類（独立系／信販系／信用系）を一覧で案内し、かぶらない保証会社なら並行審査を勧める
...
export const AIX_STAFF_NOTES: Record<string, string> = {
  ...
  guarantor_info:          "AIX【保証会社について】を押してください: お客様が審査・保証会社の不安を出した／保証会社を尋ねた／複数物件の保証会社を伝える場面で、管理会社に保証会社を確認した後に押します。物件ごとに①物件名②保証会社名③種類（独立系・信販系・信用系）を入れて「文面を作る」か「会話を合わせる」を押すと、保証会社一覧と審査の通りやすさ（独立系＝審査基準が緩い）を1通で案内します。「並行して審査かける」ONで、保証会社がかぶっていない物件の並行審査を勧める文が入ります（会社名・種類は入力値のみ・審査通過の断言はしない）",
};
