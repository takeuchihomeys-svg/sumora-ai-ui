...
const SCENE_LABEL: Record<string, string> = {
  ...
  S5_time_spec: "内覧日時の指定", S6_estimate: "見積・費用", S7_condition_change: "条件の変更", S8_cost_doubt: "費用の安さへの不安・疑問", S9_cost_breakdown: "初期費用の中身の質問（家賃だけで入居できるか・含まれる項目）", S10_phone_request: "電話で話したい（電話の可否・時間の問い合わせ・電話の依頼）",
  // 2026-09-23 竹内: 室内の写真・動画・URL の依頼／送った物件の別の部屋・間取りの質問（手元の写真・室内イメージURL を AIX【物件確認した→室内写真を確認した】から送る）
  S11_other_room: "室内の写真・動画・URL の依頼、または送った物件の別の部屋・間取りの質問（手元の写真・室内イメージURL を AIX【物件確認した→室内写真を確認した】でスタッフが送る）", application: "申込",
};
