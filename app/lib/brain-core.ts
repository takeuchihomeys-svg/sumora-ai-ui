...
import {
  mergeBrainLayers, extractStrategy, sanitizeStrategyFields, strategyForPrompt, detectStrategyShift, decideStrategyRefresh, toFreshDigest, maxSignal,
  type BrainStrategy,
...
import { stripRentNegotiation, stripRentNegotiationFromList, customerAskedRentNegotiation, RENT_NEGOTIATION_AVOID_TOPICS } from "@/app/lib/rent-negotiation-guard";
...
export type SuggestedAixMeta = {
  ...
  reply_direction?: string | null;   // 返信の方向性（120字以内の1文。2026-09-13 プロンプトの「20字以内」をコードの上限に揃えた）
  ...
  /**
   * 2026-09-21 竹内「場面でいれるかどうかはブレインに判断させる。そのためにもブレインはあるのだから（文の構成等）」
   * 返信の書き出し。"かしこまりました"|"はい"|"開口語なし"|"〇〇頂きありがとうございます"|
   * "お世話になっております"|"はじめまして"|null（決められない時はコード側の既定に任せる）。
   * final-check はこの判断に沿って見る（app/lib/opener-rates.ts の judgeOpener）。
   */
  reply_opener?: string | null;
  ...
  // 2026-09-09 Fable5 往復文脈: 最新メッセージで顧客が提案物件に述べた懸念（topic=階数/広さ/費用/審査 等・object=顧客の語）。
  // reply-context.ts mergeBrainEvidence / classifyCustomerResponse の補助証拠（fresh のみ）。JSONB のため migrate 不要
  customer_concern?: { topic: string; object: string } | null;
  ...
  // 2026-09-23: 入口（返信の方向）から落とした していない約束（家賃交渉）。digest.drop に残す
  dropped_direction?: string | null;
...
■ 冒頭挨拶（許可されるのはこの2つのみ。「お待たせ致しました」は禁止語）
...
async function detectSignalBasedAixFallback(
  ...
  try {
    ...
    // applying_pattern の most_effective 最多（見積書の後は申込へではなく、お客様の反応を見て次を決める・2026-09-12 竹内）。
...
物件検索推奨度: ${searchPriority}`;
  ...
  const applyingPatternsText = applyingPatternKnowledge.length > 0
    ? `\n【申込到達パターン（applying_pattern・どの場面でどのAIXボタンが効いたかの実績）】\n${applyingPatternKnowledge.map((k) => {
      ...
      }).join("\n")}\n※aix キーの選択は、現在の会話が上記パターンのどの「場面」に該当するかを最優先の判断材料にすること。同じ場面なら実績のあるAIXボタンを選ぶ。ただし見積書の後は申込へ進めない（見積書の次の一手はお客様の反応で決める）。`
    : "";
...
${history}`;
  ...
  try {
    ...
    const parsed = parsedAny as {
      ...
      customer_concern?: { topic?: unknown; object?: unknown } | null;
    ...
    //   場面の証拠 S10（customerRequestsPhoneCall・実データ271件中30件の検出は全て電話の依頼）なら AIX なし／確認します を 電話をかける にする。
    //   ブレインが他の AIX（見積書送る等）を選んだ時はそのまま（同じ発言の別の依頼を読んだ判断を壊さない）
    if (!promiseAix && sceneEvidence?.scene === "S10_phone_request" && (finalAix === null || finalAix === "acknowledge_check")) {
      finalAix = "phone_call";
    ...
    //   既存の線は壊さない: 宣言の直後は promise:pickup が先に決める／物件送付後は台帳が履行済みにする（pending=false）／
    //   締め後の了承（closed_ack）・内覧当日（viewing_day）・持込予告（customerWillSend）・申込以降はそれぞれ先に効く。
    //   根拠: 「出次第お送りします」宣言104件の履行率 79.8%（14日以内）・brain_no_aix の取り下げ11件中6件（55%）はその後実際に送っていた。
    const pendingPickup = resolvePendingPickup(
      {
        pickupPromisedUnfulfilled: brainLedger.facts.pickupPromisedUnfulfilled,
        pickupPromisedAt: brainLedger.facts.pickupPromisedAt,
        lastPropertiesSentAt: brainLedger.facts.lastPropertiesSentAt,
      },
    ...
    const rawReplyDirection = typeof parsed.reply_direction === "string" && parsed.reply_direction.trim()
      ? absolutizeRelativeDays(parsed.reply_direction.trim(), relDayBaseMs).slice(0, 120)
      : null;
    // 2026-09-23 竹内「家賃交渉は基本できないものだからいれない。この言い回しいれないようにする」（あっぴ事例）:
    //   出所は本文（DeepSeek）ではなくブレインの返信の方向（dir）で、前回の dir を根拠に「継続する」と既成事実化していた。
    //   digest.dir は次の回の材料になるので、**保存する前に**ここで落とす（落とさないと毎回コピーされ続ける）。
    //   実送信 12,417通で「家賃の値下げをこれから交渉・確認する」未来形は0通 ＝ 誤削除0。
    //   お客様が自分から家賃の交渉を頼んだ時だけ通す（実送信1通）。
    const custRentAsk = customerAskedRentNegotiation(
      typedMessages.filter((m) => m.sender === "customer").slice(0, 6).map((m) => m.text),
    );
    const rentGuard = stripRentNegotiation(rawReplyDirection, { customerAsked: custRentAsk });
    if (rentGuard.dropped) {
      console.log("[brain-core] rent-negotiation guard dropped direction:", conversationId, rentGuard.dropped);
    }
    ...
    if ((rentGuard.dropped || keyTopicsGuard.dropped.length > 0) && !custRentAsk) {
      for (const w of RENT_NEGOTIATION_AVOID_TOPICS) avoidSet.add(w);
    }
    ...
    // 2026-09-23 反証者の指摘: 家賃交渉の2語も末尾に足していたので同じ5件上限で落ちていた（実データ90会話で57%が4〜5件＝少なくとも1語消える）。
    //   決定論の項目はここに並べて先頭固定にする（avoidSet に入っている時だけ残る）
    const deterministicAvoid = ["来阪", "見積書", "初期費用", ...RENT_NEGOTIATION_AVOID_TOPICS].filter((t) => t === "来阪" || avoidSet.has(t));
    ...
    // 来阪・決定論の項目を先頭固定で残して最大5件（末尾sliceで落ちるのを防ぐ）。見積書送るの時に上で外した項目は avoidSet に無いので戻らない
    const avoidTopics = [...deterministicAvoid.filter((t) => t === "来阪" || avoidSet.has(t)), ...Array.from(avoidSet).filter((t) => !deterministicAvoid.includes(t))].slice(0, 5);
    ...
    const CONCERN_TOPICS = new Set(["階数", "築年数", "費用", "駅距離", "広さ", "日当たり", "審査", "騒音治安", "家族構成", "設備", "ペット駐車場", "時期"]);
    const customerConcern: { topic: string; object: string } | null = (() => {
      const c = parsed.customer_concern;
      if (!c || typeof c !== "object") return null;
      const topic = typeof c.topic === "string" ? c.topic.trim() : "";
      const object = typeof c.object === "string" ? c.object.trim().slice(0, 40) : "";
      if (!CONCERN_TOPICS.has(topic) || !object) return null;
      return { topic, object };
    })();
    ...
    // 2026-09-12 竹内（KENYOU 事例）: 最新の顧客発言が送付物件の一部の見送り（「フジパレスは無しで」）なら、
    //   その発言に出てこない物件（残りの送付物件）を話題の中心にしない。1件を外した＝残りを選んだ、ではない
    const latestCustomerForPass = typedMessages.find((m) => m.sender === "customer")?.text ?? "";
    const latestPropertyPass = detectPropertyPass(latestCustomerForPass, {
      recentStaffText: typedMessages.find((m) => m.sender === "staff")?.text ?? "",
    });
    const currentProperty = rawCurrentProperty && propertyNameCore &&
      ...
      !(latestPropertyPass && !latestCustomerForPass.includes(propertyNameCore))
      ? rawCurrentProperty
      : null;
    ...
    // 2026-09-12 竹内（KENYOU 事例）: 送付物件の一部の見送りは「決定（decision）」ではない（残りの物件を選んだとは言っていない）→ desire（探索継続の希望）
    const customerIntentFinal: NonNullable<SuggestedAixMeta>["customer_intent"] | null =
      conditionChangeType !== null ? "desire"
      : latestPropertyPass && (customerIntent === "decision" || customerIntent === "positive") ? "desire"
      : customerIntent;
...
async function consolidateStrategy(conversationId: string, conv: Record<string, unknown>, prev: BrainStrategy, nowIso: string): Promise<BrainStrategy | null> {
  ...
  // 2026-09-23 反証者の指摘: 戦略の項目は家賃交渉の守りを通っていなかった（毎ターン fresh を上書きする一番長く居座る層）。
  //   extractStrategy と同じ sanitizeStrategyFields を通す。お客様が自分から頼んだ時（実送信1通）は残す
  const strategyCustomerAsked = customerAskedRentNegotiation(msgs.filter((m) => m.sender === "customer").map((m) => m.text));
  const guardedFields = sanitizeStrategyFields({
  ...
  }, { customerAsked: strategyCustomerAsked });
 ...
 *   - 保存済みの本分析（cached 以外）の方が新しいお客様発言を見ている → 書かない（古い判断で上書きしない）。newer=true
 *     yieldToEqual: 同じ発言を見た本分析があれば譲る（分析の省略の結果で本分析を上書きしない）
 * 行の updated_at は分析と関係ない更新（下書き・フラグ・sync-from-screening）でも変わるので使わない（設計知見「古い結果で上書きしない」は入力の時刻で比べる）
 */
async function brainWriteBlock(
  conversationId: string,
  ourAnalyzedTs: string | null | undefined,
  analysisStartedAt: string,
  opts: { yieldToEqual: boolean },
): Promise<{ reason: string; newer: boolean; existingTs: string | null } | null> {
  const ourTs = ourAnalyzedTs ? Date.parse(ourAnalyzedTs) : NaN;
  ...
  if ((staffSentDuring?.length ?? 0) > 0) return { reason: "staff_sent_during_analysis", newer: false, existingTs: null };
  if (nowStatus && BRAIN_SKIP_STATUSES.includes(nowStatus)) return { reason: `status=${nowStatus}`, newer: false, existingTs: null };
  const existing = (currentRow?.suggested_aix_meta ?? null) as { analyzed_msg_ts?: string | null; source?: string } | null;
  const existingTs = existing?.analyzed_msg_ts ? Date.parse(existing.analyzed_msg_ts) : NaN;
  if (Number.isFinite(existingTs) && existing?.source !== "cached"
    && (!Number.isFinite(ourTs) || existingTs > ourTs || (opts.yieldToEqual && existingTs >= ourTs))) {
    return { reason: "newer_analysis_exists", newer: true, existingTs: existing?.analyzed_msg_ts ?? null };
  }
  return null;
}
...
async function analyzeAndSaveBrainMetaInner(
  ...
  if (analysisMode === "cached") {
    ...
    // 2026-09-13 監査 抜け3: note・reason・template_hint も前回の action に紐づく「前の発言についての判定」なので持ち越さない
    //   （旧: action だけ空にして note は残し、返信生成が「必須の WE DO 宣言」として前回の行動を入れていた）
    ...
    // 2026-09-13 監査 抜け4: 省略の書き込みにも旧ウォーターマーク（updated_at 一致）が残っていて、sync-from-screening 等の
    //   無関係な更新で捨てられ、短い返事（お願いします等）の返信生成が判断なし（T3）になっていた。本分析と同じ見送り判定に揃える。
    //   同じ発言を見た本分析がすでにあれば、省略の結果で上書きしない（yieldToEqual）
    const cachedBlock = await brainWriteBlock(conversationId, cachedResult.analyzed_msg_ts, analysisStartedAt, { yieldToEqual: true });
    if (cachedBlock) {
      console.log(JSON.stringify({ tag: "brain:write-skip", conversationId, reason: cachedBlock.reason, mode: "cached" }));
  ...
  const block = await brainWriteBlock(conversationId, metaToWrite.analyzed_msg_ts, analysisStartedAt, { yieldToEqual: false });
  if (block && !block.newer) {
    console.log(JSON.stringify({ tag: "brain:write-skip", conversationId, reason: block.reason }));
    await supabase.from("conversations").update({ brain_analyzed_at: new Date().toISOString() })
      .eq("id", conversationId)
      .or(`brain_analyzed_at.is.null,brain_analyzed_at.lt.${new Date(Date.now() - 25 * 60 * 1000).toISOString()}`);
  ...
  if (block?.newer) {
    console.log(JSON.stringify({ tag: "brain:write-skip", conversationId, reason: block.reason, ours: metaToWrite.analyzed_msg_ts ?? null, existing: block.existingTs }));
