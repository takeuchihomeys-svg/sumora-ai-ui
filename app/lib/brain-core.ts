...
ルール⑥（短い感謝・了承には短く返す）: 顧客の最新メッセージが感謝・了承のみ（「ありがとうございます」「了解です」「わかりました」「はい！」「承知しました」「よろしくお願いします」等、40字未満かつ質問・要望・懸念を一切含まない）の場合 → reply_direction を「短く感謝を受け取る」にする。avoid_topics に「検討依頼の繰り返し」「物件追加提案」「費用説明」を追加する。key_topics は空にする（余計な話題を押しつけない）。直前スタッフ発言に既に「ご検討ください」「検討よろしくお願いします」等の締め文句がある場合は特にこのルールを厳守する（同じ内容の繰り返しはしつこさになる）。迷った時: メッセージに「〜はどうですか」「〜を教えてください」「〜したい」等の要求・質問が1つでもあればこのルールを適用しない（質問への回答を優先する）。

ルール⑦（お客様の返信の短さに合わせる）: 顧客の最新メッセージが20字以内の場合、reply_direction に「簡潔に」を付け加える（例: 「簡潔に内覧日確認」「簡潔に感謝を受け取る」）。顧客が短く返している場面で長文を送ることは温度感のミスマッチになる。迷った時: 顧客メッセージが質問を含む場合は適用しない（質問には丁寧に答える）。
