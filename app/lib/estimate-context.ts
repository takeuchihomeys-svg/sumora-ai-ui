...
export function buildEstimateGateNote(v: EstimateContextVerdict | null): string {
  ...
  if (v.mode === "declare") {
    return (
      ...
      // 2026-09-23 S2 の実測（こちらの🌟物件カード直後の費用の質問・生成3回）: 3/3 が「初期費用確認させて頂きます／確認出来次第ご連絡」だけで
      //   御見積書に触れなかった。実送信の同場面は 御見積書の宣言 71%（過半数）・「最大限割引」50%・「確認します」だけで終える形 29%。
      //   宣言は必須にしてよい（過半数）。「確認します」は禁止ではなく、御見積書の宣言と両立させる
      (v.trigger === "customer_cost_question" && v.sentPropertiesCount > 0
        ? "こちらが送った物件の費用を聞かれた実送信は **御見積書の宣言 71%・「最大限割引」50%**。「初期費用確認させて頂きます／確認出来次第ご連絡」だけで終える形は 29% しかない → 確認の一文を書く時も「最大限割引させて頂いた御見積書お送りさせて頂きます」の宣言を必ず添える。"
        : "") +
