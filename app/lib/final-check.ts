...
export interface FinalCheckContext {
  /** 2026-09-22 竹内（𝓡さん事例）: お客様が送ってきた物件が全部こちらの送った物件（own-property-server の照合・生成と同じ結果） */
...
const BANNED_PATTERNS: BannedPattern[] = [
  ...
  //     このような場面で使わない」───────────────────────────────────────────
  //   実物（ギガ賃貸 9/21）: 物件の詳細＋見積書を送った直後の「ありがとうございます🙇 検討します」に
  //   「お申込みいただきありがとうございます😊」。申込は1件も入っていない＝**事実と違う**。
  //   実測（scripts/audit-apply-thanks.ts・直近365日）:
  //     お礼の形「お申込み…ありがとう」は実送信 **0通 / 6,816通（0.00%）** ＝ スタッフは一度も使わない
  //     依頼の形「お申込みいただき、ご内覧設定させて…」は 20通（別物なので当てない）
  //     手本（ai_reply_examples）0件 ／ ナレッジ（11,207件）0件 ＝ 写したのではない
  //   根本原因（TPO の誤判定）は negative-context.ts で直したが、ここは**二重の守り**。
  //   ⚠ 申込が実際に進んでいる会話では正しい文なので skipIfHistRe で免除する。
  { re: /お?申込(?:み|)(?:いただき|頂き|下さり|くださり)?(?:誠に)?ありがとう/, code: "APPLY_THANKS_NO_APPLICATION",
    msg: "申込へのお礼だが、この会話に申込の事実が無い（申込は審査のお申込みの時に使う語）",
    sug: "「お申込みいただきありがとうございます」を削除。検討中なら「ごゆっくりご検討ください」、これから申し込んでもらうなら「お気に召されましたらお申込みでお部屋抑えさせて頂きます」に変更",
    skipIfHistRe: /お申込(?:み)?(?:完了|手続き|させて(?:頂|いただ)き|進め|入(?:り|って))|審査(?:中|結果|進め|に進)|申込書|1番手|一番手|お部屋(?:を)?(?:抑え|押さえ)(?:させて|ました)/,
    blockAlways: true },
...
export async function runFinalCheckWithRevision(
  ...
  if (blocks1.length === 0) {
    ...
    const revised = applySurfaceFixes(revisedRaw, { customerName: ctx.customerName ?? "", aliases: ctx.nameAliases, now: ctx.now, customerMessage: ctx.lastCustomerMessage, customerMessageAt: lastCustomerMessageAtOf(ctx), ownPropertyReturnedAll: ctx.ownPropertyReturnedAll }).text;
