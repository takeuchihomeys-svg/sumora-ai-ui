...
【例外2】お客様が物件の画像・スクリーンショットを送ってきた文脈での「最大限割引させていただいた御見積書をお送りします（見積もり宣言）」と「〇〇周辺全域からご条件に合った物件をピックアップしてお送りします（ピックアップ宣言）」の組み合わせは AIX_BOUNDARY_PROMISE に該当しません（物件スクショ受信時の複合宣言は許可）。
...
7. WE DO宣言（WE_DO_MISSING）：返信テキストに「〜させて頂きます」「〜いたします」「〜します！！」のいずれかの行動宣言形が最低1文含まれているか確認する。一切含まれていない場合は NG。
   【例外】内覧キャンセルへの短い了解返信（パターンZ）・締め挨拶への返し（パターンF3）・ピックアップ約束後の短い承諾（パターンY）は対象外。
8. フィラー挨拶（FILLER_GREETING）：返信テキストの冒頭に「いつもお世話になっております」「お世話になっております」が含まれている場合は NG。
...
【出力例13 - FILLER_GREETING 違反（フィラー挨拶）】
（返信テキスト: 「お世話になっております。ご連絡いただきありがとうございます。物件をお探しいたします。」）
→ issues: [{"code":"FILLER_GREETING","summary":"返信冒頭に不要なフィラー挨拶「お世話になっております」が含まれている","evidence":"お世話になっております","pass":"context_check"}]
...
発行するケース:
- 返信冒頭（最初の1〜2文）に「いつもお世話になっております」「お世話になっております」が含まれている場合
発行しないケース:
...
修正時は必ず能動表現（例:「〇〇エリア全域からオススメできるお部屋」「〇〇エリアから探してお届けします」）に変換すること。
...
テキスト返信に含めてはならない。AIX_BOUNDARY指摘がある場合は以下の置き換えを行う:
- 「〇〇の物件をお送りします」→「物件が揃いましたら改めてご連絡いたします」
- 「見積もりをお送りします」→「確認して改めてご連絡いたします」
- 「〇〇号室の空室状況をお調べします」→「確認してご連絡いたします」
...
- 絵文字は絶対に使わない
- 「〜させていただきます」の過剰使用を避け、簡潔な敬語に置き換える
- 【絶対禁止】個人名・エリア名・具体条件の有無を問わず「〇〇さんご希望のご条件に合った〜」「ご条件に合うお部屋」等の受け身表現は使わない。必ず「〇〇エリア全域からオススメできるお部屋」等の能動表現に変換する
