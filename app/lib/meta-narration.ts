...
const META_LINE_RES: RegExp[] = [
  ...
  // 2026-09-26 YUMA の DeepSeek 実測（段3の生成の確かめ）1行目「【AIX実行済み・確認結果報告済みを踏まえた返信】」
  //   ＝プロンプトの節の名前（AIX実行済み再宣言ブロック）を見出しにした作業メモ。「場面/状況」を含まない形。
  //   1行まるごと【…返信/返答/回答/お返事（案/文）】の行は、スタッフの実送信（「】」を含む943通）・手本の下書き（176通）で0件（誤削除0）
  /^\s*【[^】\n]{0,80}(?:返信|返答|回答|お返事)(?:案|文)?】\s*$/,
