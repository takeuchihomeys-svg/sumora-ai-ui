 ...
 * ＝ 誤読を DB に入れない側に倒す。
 *
 * 【辞書は「その会話の物件名」に限る — 全社の辞書は使わない】
 *   2026-09-20 に全社辞書（sent_properties 全件 5,784種類）と比べた（scripts/audit-property-dict.ts）。
 *   実画像20枚で:
 *     会話だけの辞書で照合を通る   17枚(85%)
 *     全社の辞書で照合を通る       17枚(85%)  ← **同じ。得る物が無い**
 *     全社辞書だけで救えた           1枚      ← **しかもそれが誤照合だった**
 *       「グランパシフィック生野東」→「グランパシフィック**梅南**」(0.76)
 *       ＝ シリーズ名が共通だと別の物件に寄ってしまう。
 *   お客様に送った物件を別の物件として記録すると、ブレインが違う物件で文を書く。
