...
export const PAIR_MATRIX: PairRule[] = [
  ...
  { id: "ES_POSITIVE", staff: "estimate_send", customer: "positive", precedence: "after_wait",
    ...
    direction: "見積への前向き反応を受け止め、お気に召されたお部屋を実際にご案内する提案1文。申込へは進めない（お客様が自分から申込を口にした時だけ申込の話をする）。煽り禁止。70〜140字",
    mustInclude: [{ label: "内覧のご案内", detect: /ご案内|ご内覧|内覧|内見/,
      fix: "「{name}お気に召されたお部屋ご都合よろしいお日にちにご案内させて頂きます😊！！」を1文入れる" }],
    mustNot: ["お客様が申込を口にしていないのに申込を促す", "「埋まってしまいます」等の煽り", "書類リスト生成", "未確認の退去日・入居可能日の断言"],
