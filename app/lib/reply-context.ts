...
export function staffEstimateDelivered(text: string): boolean {
  // 「ご査収いただきありがとうございます」は前に送った物への感謝（この1通の送付の印ではない）
  const t = (text ?? "").replace(/ご査収(?:いただ|頂)き(?:まして)?ありがと[^\n。！!]*/g, "");
  ...
  return t.split(/\n|(?<=[。！!？?])(?![。！!？?])/).some((s) => /(?:御|お)?見積(?:書|り|もり)/.test(s)
    ...
    // 「御見積書、現在確認中となります」「作成中」は約束・途中（送った印ではない）
    && !/作成|お送り(?:させて(?:頂|いただ)きます|いたします|致します|します)|お作り|確認中|作成中|準備中|手配中/.test(s));
}
...
//   「ご希望条件に合ったお部屋ピックアップしてお送りさせて頂きます」（ピックアップ宣言）まで条件ヒアリングにしていた → 尋ねる・入力を頼む形だけにする
// 2026-09-12 竹内（響夢事例）: 「こちらに見木さん最新のお部屋探しの条件お送り頂きますと…ピックアップしお送りさせて頂きます😊！！
//   よろしければお手隙の際にご入力ください」は条件の送付を条件にした記入のお願い＝条件ヒアリング。旧はピックアップ宣言になり、
//   ブレインが 物件ピックアップした をセットしたまま止まった（条件はまだ届いていない）。
//   「条件(を)お送り頂きますと／頂けましたら」「お部屋探し(の)フォーマット・条件…お送り頂けましたら／ご入力」を足す。
//   150日で分類が変わる17件は全て条件のお願い（うち13件はその後お客様が条件を送った）。「ご条件お送り頂きありがとうございます」（受領後の本当の宣言）・
//   申込フォームの「ご入力ください」は含めない
export const STAFF_CONDITION_ASK_RE = /お部屋お探し中|ご希望のお部屋探しご条件|①【?ご入居(?:の)?時期|ご希望(?:の)?(?:ご)?条件(?:を|について|など|等)?(?:お聞かせ|教えて|お伺い|伺わせ|お送り(?:ください|下さい|頂け|いただけ)|ご入力|ご記入)|ご希望の(?:エリア|地域)(?:を|は|など)?(?:お聞かせ|教えて|お伺い)|条件(?:を)?お聞かせ|ご(?:入力|記入)(?:頂|いただ)|条件(?:を)?お送り(?:頂|いただ)(?:きますと|けますと|けましたら|きましたら|けたら|ければ)|お部屋探し(?:の)?(?:フォーマット|ご?条件)[^\n]{0,12}(?:お送り(?:頂|いただ)(?:きますと|けますと|けましたら|きましたら|けたら|ければ)|ご(?:入力|記入))/;
...
/**
 * (A') 手元に物件がある（他社で内覧した・見つけた・気に入った）と言い、初期費用・見積・確認を頼んだが、まだ物件を送っていない。
 *   2026-09-12 竹内（あや事例）「他の不動産屋さんで内覧したお家があり、良さそうだなって思った物件がありまして、初期費用がどれくらいに
 *   なるか教えていただきたいのですが、可能でしょうか？」→ 実送信「あやさんお気に召されましたお部屋お送り頂きますと最大限割引しました
 *   初期費用御見積させて頂きます！！」（Sana 事例「初期費用など諸々を調べて頂きたい物件があるのですが」→「お部屋お送り頂きましたら
 *   物件の募集状況の確認と最大限割引しました初期費用の御見積書作成しお送り」も同じ）。
 *   旧: 持込予告と判定できず質問扱い →「お送り頂きました物件の初期費用確認させて頂きます」（届いた前提）・ブレインは見積書送る。
 *   お客様がこれから送る＝(A) 持込予告と同じ流れ（届いたら募集状況確認＋最大限割引した初期費用の御見積書）
 */
// 「気になる物件がありましたら／あれば」（仮定）・「ありません」（否定）は手元の物件ではない
//   「いい物件があり次第」（条件フォームの入居時期）・「空き物件があるか調べて」（あるかどうかの質問）も手元の物件ではない
const CUST_HOLDS_PROPERTY_RE = /(?:物件|お?部屋|お家|お?うち|マンション|アパート|戸建)(?:が|も)(?:あり(?!ません|ましたら|ませんでした|次第)|ある(?!なら|か)|あって|ございま(?!せん|したら))/;
const CUST_HOLDS_ASK_RE = /初期費用|見積|費用|調べて|確認して|空いて|募集|取り?扱/;
/** (A) 持込予告か（(B)依頼形は除外）。route.ts の estimate 入力・final-check E5 フォールバック・場面の証拠・物件確認の依頼判定・
 *  webhook のタスク検知・画面のタスク自動作成・ブレインの AIX なし判定が同一判定を使う。
 *  物件そのもの（URL・画像）が同じ連投にある時は予告ではなく到着（呼び出し側が URL・画像の有無で除外する） */
export function CUST_WILL_SEND_SELF_PRED(text: string): { yes: boolean; evidence: string | null } {
  ...
  if (line) return { yes: true, evidence: line.slice(0, 40) };
  // (A') 手元の物件＋見積・確認の依頼（改行をまたいで書かれるので1本につなげて見る）
  const flat = t.replace(/\s+/g, "");
  if (!/https?:\/\/|\[画像\]/.test(t) && !isConditionFormMessage(t)) {
    const hold = CUST_HOLDS_PROPERTY_RE.exec(flat);
    if (hold && CUST_HOLDS_ASK_RE.test(flat)) return { yes: true, evidence: flat.slice(Math.max(0, hold.index - 12), hold.index + 16) };
  }
...
export function classifyCustomerResponse(
  ...
  //   感謝もピックアップ宣言も無い返信になっていた。判定は isConditionFormMessage（greeting の感謝判定と同じ）
  if (isConditionFormMessage(raw)) put("condition_change", "regex:condition_form");
  ...
  // 2026-09-12 竹内（あや事例）: 手元の物件＋見積・確認の依頼（まだ送っていない）は改行をまたぐので行ごとの判定では拾えない → 全体で判定
  const holdsUnsent = !found.has("will_send_later") && !/https?:\/\/|\[画像\]/.test(raw) && CUST_WILL_SEND_SELF_PRED(raw).yes;
  if (holdsUnsent) put("will_send_later", `holds_property:${CUST_WILL_SEND_SELF_PRED(raw).evidence ?? ""}`);
  ...
  // ⑤'' 手元の物件の「初期費用を教えていただきたいのですが、可能でしょうか？」は、その物件を送ってもらう流れの依頼（質問への回答ではない）。
  //    質問の行が1つまでの時だけ question を落とす（別の質問が並んでいる時は残す。正規化で文中の「？」は消えることがあるので行で数える）
  if (holdsUnsent && found.has("question") && lines.filter((p) => CUST_QUESTION_RE.test(p)).length <= 1) found.delete("question");
...
export function viewingOfferLiteral(
  customerName: string,
  named: boolean,
  count = 1,
  // 2026-09-20 竹内（まりあさん事例）: 内覧が既に決まっている時は [Y] 型を出さない（下の fixedViewingOfferLiteral を参照）
  appointment?: FixedViewing | null,
): string {
  if (appointment) return fixedViewingOfferLiteral(customerName, appointment, count);
...
// ─────────────────────────────────────────────────────────────
// 内覧が既に決まっている時のご案内文（2026-09-20 竹内・まりあさん事例）
//
// 症状: 9/21 16:00 の内覧が決まっているまりあさんの「全て見に行きたいです！」に対し、
//   AI が「かしこまりました！！ お申込み情報受け取りました😊！！…」と、受け取ってもいない申込を捏造した。
//   スタッフの正解は「かしこまりました！！ 明日全てご案内させていただきます！！」。
//
// 原因: 生成に渡っていた2つの必須が**正面衝突**していた（材料は全て正しく渡っていた・他人の会話の混入は0）。
//   行動台帳 : 「この内覧は決まっている。新しい内覧日程の打診（…「ご案内させて頂きます」）は書かない」
//   ES_POSITIVE: 「『ご都合よろしいお日にちにご案内させて頂きます😊！！』を1文入れる」（必須）
//   書くなと書けを同時に渡され、LLM はどちらも避けた第三の文を作った。
//
// 実データ（scripts/audit-fixed-viewing-reply.ts）:
//   内覧が決まっている間のスタッフ返信で「ご都合よろしいお日にち」型の新規打診は **0/6（0%）**。
//   正解形は決まっている日時をそのまま言う: 「本日12時…お部屋ご案内させて頂きます！」「本日16時お部屋ご案内させて頂きます！」
//   → 禁止すべきは「ご案内させて頂きます」ではなく「日時を決めずに新しく打診すること」だった（台帳の注記も同時に直す）。
// ─────────────────────────────────────────────────────────────
/** 行動台帳が持つ「決まっている内覧」（action-ledger.ts の LedgerFacts['viewingAppointment']） */
export type FixedViewing = NonNullable<ActionLedger["facts"]["viewingAppointment"]>;

/** 「16:00」→「16時」／「16:30」→「16時30分」。読めない時は空（日時を作らない） */
export function formatAppointmentTime(time: string | null | undefined): string {
  const m = /^(\d{1,2}):(\d{2})/.exec(String(time ?? "").trim());
  if (!m) return "";
  const h = Number(m[1]), mi = Number(m[2]);
  if (!Number.isFinite(h) || h > 23 || mi > 59) return "";
  return mi === 0 ? `${h}時` : `${h}時${mi}分`;
}

/** 決まっている内覧の案内文。日時は台帳の値だけを使う（創作しない・読めない時は日時を書かない） */
export function fixedViewingOfferLiteral(customerName: string, a: FixedViewing, count = 1): string {
  const n = customerName ? `${customerName}さん` : "";
  const obj = count >= 2 ? `${count}部屋` : "お部屋";
  const day = a.day === "today" ? "本日" : a.day === "tomorrow" ? "明日" : (a.day === "later" && a.dateMD) ? a.dateMD : "";
  const when = `${day}${formatAppointmentTime(a.time)}`;
  // 日時が1つも読めない時は「決まっている」事実だけを言う（「ご都合よろしいお日にち」には絶対に戻さない）
  return `${n}${when}${obj}ご案内させて頂きます！！`;
}
...
// ─── 2026-09-20 竹内「次の一手の必須を緩める」───────────────────────────────────
//  実送信と突き合わせた監査（scripts/audit-cell-vs-sent.ts）で、検討中・了承・懸念の場面のセルが
//  **スタッフが実際に書く形を取りこぼしていた**（VI_THINKING 25% / ES_THINKING 33% / PS_THINKING 40%）。
//  原因は detect が「ごゆっくり」1語だけだったこと。スタッフは同じ意味を別の言い方で書いている:
//    「お手隙の際にご確認よろしくお願い致します😊！！」
//    「〇さんお手隙の際にご確認いただけますと幸いです😊！！」
//    「ぜひゆっくりご覧いただけますと幸いです😌！！」
//  必須を外すのではなく**同じ意味の実文を認める**（外すと修正ループの材料まで消える）。
/** 急かさず待つ姿勢（3セルが共有＝四者同名。実送信の形だけを入れる） */
export const GENTLE_WAIT_RE =
  /ごゆっくり|お手隙の際|ゆっくりご(?:覧|確認|検討|相談)|お時間のある時|お急ぎでな/;

/** 扉を開けたまま待つ1文の**必須要素の detect**（締めの種別を決める OPEN_DOOR_RE とは別物）。
 *  OPEN_DOOR_RE は「いつでもお気軽にご連絡ください」の完成形だけを見る厳しい判定で、
 *  締めが open_door かを決めるためのもの。こちらは**必須を満たしたか**を見るので実送信の形を広く採る。
 *  2026-09-20: 旧 detect は `/内覧|いつでも/` で、スタッフが実際に書く「**何時でも**」（漢字）を
 *  取りこぼしていた（実物「気になる点出てきましたら何時でもお気軽にご連絡ください😌！！」）。 */
export const DOOR_OPEN_ANY_RE =
  /いつでも|何時でも|お気軽|お待ちしております|お待ちしてます|随時|出てきましたら|出次第|で次第|内覧/;
...
export const PAIR_MATRIX: PairRule[] = [
  ...
  { id: "CA_CONDITION", staff: "condition_ask", customer: "condition_change", precedence: "override_wait",
    ...
    direction: "我々の条件ヒアリングに対しお客様が条件（フォーム／箇条書き）を返した。①フォーム（①〜⑧）で返してくれた時は必ず「ご条件お送り頂きありがとうございます😊！！」（フォームへの感謝。かしこまりましたではない）／文章・箇条書きの条件なら「かしこまりました！！」 ②お客様の言葉のままエリア・家賃・間取りを復唱したピックアップ宣言1文（物件名・建物名は書かない。「〜周辺全域から{name}にオススメできるお部屋ピックアップさせて頂きます！！」） ③任意「ピックアップ出来次第お送りさせて頂きます！！」 ④任意「何卒よろしくお願い致します！！」。96〜143字",
    mustInclude: [
    ...
    example: "ご条件お送り頂きありがとうございます😊！！\n\n梅田まで1本で行ける沿線周辺全域から9万以内・1LDK・カウンターキッチン希望・築10年以内で{name}にオススメできるお部屋ピックアップしてお送りさせて頂きます！！\n何卒よろしくお願い致します！！",
  ...
  { id: "PD_CONDITION_CHANGE", staff: "pickup_declared", customer: "condition_change", precedence: "override_wait",
    ...
    example: "かしこまりました😊！！\n\n大阪市内に絞らせて頂き、梅田まで1本で行ける沿線・9万以内・1LDK・カウンターキッチン希望・築10年以内で{name}にオススメできるお部屋をピックアップさせて頂きます！！\n\n{name}にオススメ出来るお部屋ピックアップ出来次第お送りさせて頂きます！！何卒よろしくお願い致します！！",
  ...
  { id: "PD_ACK", staff: "pickup_declared", customer: "ack_only", precedence: "after_wait",
    ...
    example: "はい😊！！\n{name}にオススメ出来るお部屋ピックアップ出来次第お送りさせて頂きます！！",
  ...
  { id: "PD_QUESTION", staff: "pickup_declared", customer: "question", precedence: "override_wait",
    ...
    example: "トイレと洗面所別のお部屋につきましては、設備分家賃が高くなる傾向がございます！！(3,000円～5,000円程）\n\nトイレ・洗面所別のご条件も含めて{name}にオススメ出来るお部屋ピックアップ出来次第お送りさせて頂きます！！何卒よろしくお願い致します！！",
  ...
  { id: "ANY_CONDITION_CHANGE", staff: "*", customer: "condition_change", precedence: "override_wait",
    ...
    example: "かしこまりました！！\n2LDKのご条件で、枚方・高槻・吹田・守口・門真・鶴見区周辺全域から{name}にオススメ出来るお部屋新たにピックアップしてお送りさせて頂きます😌！！\n{name}にご満足頂けるお部屋が見つかるまで全力でサポートさせて頂きます！！",
    exampleBySent: {
  ...
  { id: "VI_CONCERN", staff: "viewing_invite", customer: "concern", precedence: "override_wait",
    ...
    example: "{name}お世話になっております！！\nご要望お聞かせ頂きありがとうございます😊！！\n新生児のお子様との階段の上り下りはご負担になりますので、1階またはエレベーター付きのお部屋を中心に{name}にオススメできるお部屋再度ピックアップしお送りさせて頂きます！！\nこちらのお部屋も含めお気に召されましたらいつでもご内覧頂けますので、{name}にご満足頂けるお部屋が見つかるまで全力でサポートさせて頂きます😌！！",
  ...
  { id: "VI_POSITIVE", staff: "viewing_invite", customer: "positive", precedence: "after_wait",
    ...
    // 2026-09-20 竹内（まりあさん事例）: 台帳に決まっている内覧がある時に新しく日程を聞くと台帳の禁止と衝突する
    // 2026-09-20 竹内（YUMA 本番検証）「AIX から返信する部分は AIX からスタッフが送るから大丈夫」:
    //   旧 direction/fix は「候補日時を出していなければ『ご都合よろしいお日にち御座いますでしょうか』を書け」と
    //   指示していたが、実送信46通・AIX 23件（全部 viewing_invite）は**全部が具体日時とセット**だった
    //   （「直近ですと 9/21(月) 12:00〜16:00 …ご案内可能です😊！！ ご都合よろしいお日にち御座いますでしょうか」）。
    //   設計知見も「[X] は96%が具体日時とセット・AIX【内覧日調整】専用・通常返信で書かせてはならない」。
    //   日時を出すのは AIX の仕事なので、**本文は受付だけ**にする（日時も疑問形も書かせない）。
    direction: "内覧受付。行動台帳に決まっている内覧があればその日時の復唱のみ（新しい日程は聞かない）。無い場合は「かしこまりました！！ お部屋ご案内させて頂きます！！」の受付だけで終える。**候補日時の提示も「ご都合よろしいお日にち御座いますでしょうか」の確認も AIX【内覧日調整】の担当なので本文には書かない**（スタッフが AIX から送る）。既に候補日を出している場合は日時確定の復唱のみ。40〜90字",
    mustInclude: [
      { label: "受付語", detect: /かしこまりました/, fix: "「かしこまりました！！」を単独行で置く" },
      // 2026-09-20: 日時を書かない「受付だけ」でも満たせるようにする（日時は AIX の担当）
      { label: "受付 or 日時の確定復唱", detect: /ご案内(?:させて(?:頂|いただ)き|いたし|致し)ます|ご都合|お日にち|[0-9０-９]{1,2}[/／月][0-9０-９]{1,2}|[0-9０-９]{1,2}時/,
        // 2026-09-20 竹内（まりあさん事例）: 台帳に決まっている内覧がある時にここで新しく日程を聞くと、
        //   台帳の「新しい日程の打診は書かない」と衝突する。決まっているなら復唱が先（日時は台帳の値だけ）
        // 2026-09-20 竹内（YUMA 本番検証）: 日時の提示も「御座いますでしょうか」も AIX【内覧日調整】の担当。
        //   本文は受付だけにする（実送信46通は全部が具体日時とセットで、日時なしのこの疑問形は0通）
        fix: "行動台帳に決まっている内覧があればその日時をそのまま復唱する。無い場合は「かしこまりました！！ お部屋ご案内させて頂きます！！」の受付だけで終える（候補日時も「ご都合よろしいお日にち御座いますでしょうか」も書かない＝AIX【内覧日調整】でスタッフが送る）" },
    ],
    mustNot: ["申込誘導", "別物件提案", "募集未確認物件への内覧確約"],
    // 2026-09-20 竹内（YUMA 本番検証）: 例文から [X] 型の疑問形を外す（日時は AIX【内覧日調整】の担当）
    example: "かしこまりました！！\nお部屋ご案内させて頂きます！！",
  ...
  { id: "PS_POSITIVE", staff: "property_send", customer: "positive", precedence: "override_wait",
    ...
    // 2026-09-20 竹内（まりあさん事例）: 例文のリテラルも {viewingOffer} に寄せる。
    //   fix だけ直しても、例文が「ご都合よろしいお日にちに」を持っていると
    //   内覧が決まっている会話で台帳の「新しい日程は打診しない」と衝突したままになる
    example: "かしこまりました！！\n{viewingOffer}",
  ...
  { id: "VI_THINKING", staff: "viewing_invite", customer: "thinking", precedence: "after_wait",
    ...
    mustInclude: [
      { label: "急かさない受け止め（顧客が使った動詞をそのまま鏡写しにする）", detect: GENTLE_WAIT_RE, when: hasGoyukkuriMirrorVerb,
        fix: "顧客が『検討します』なら「ごゆっくりご検討頂けますと幸いです😊！！」／『確認します』なら「ごゆっくりご確認頂けますと幸いです😊！！」" },
      // 2026-09-20: 「何時でも」（漢字）「お気軽に」も同じ扉の開け方として認める（OPEN_DOOR_RE）
      { label: "内覧の扉を開ける1文", detect: DOOR_OPEN_ANY_RE,
        fix: "「ご内覧出来ますので、気になる点出てきましたらいつでもお気軽にご連絡ください😌！！」を1文入れる" },
    ],
    mustNot: ["希少性煽り", "申込誘導", "物件追加提案", "「かしこまりました」開口語", "顧客が言っていない動詞での「ごゆっくり〇〇」"],
    example: "はい😊！！\nごゆっくりご検討頂けますと幸いです！！\nご内覧出来ますので、{name}気になる点出てきましたらいつでもお気軽にご連絡ください😌！！",
  ...
  { id: "ES_THINKING", staff: "estimate_send", customer: "thinking", precedence: "after_wait",
    ...
    // ─── 2026-09-21 竹内「申込ではない場面で申込とか出るのは防ぐ必要あるから」───────────
    //   ここには「申込」の語が **direction と label の2か所**にあり、
    //   同じセルの mustNot では「申込催促・希少性煽り」と**書くなとも言っていた**
    //   （設計知見「同じ事実について『書くな』と『書け』を別の場所から渡さない」）。
    //   例文（example）にも実際の fix にも「申込」は1文字も無く、書いてほしいのは**内覧のご案内**。
    //   実測（scripts/audit-apply-word-scene.ts・直近365日）:
    //     お客様が「検討します／確認します」と言った 179通のうち返信に「申込」があるのは 19通（10.6%）。
    //     ただし全部が「これから申し込む提案」か「制度の説明」で、
    //     **「済んだことへのお礼」は 0通**（そちらは final-check の APPLY_THANKS_NO_APPLICATION で止めた）。
    //     そして **AI がこの場面で書いた「申込」13通のうち 8通（61.5%）をスタッフが消している**。
    //     消した後に入るのは「ご案内（内覧）」「ご不明な点ございましたら」。
    //   → ラベルと direction から「申込」を外す。**検出（detect）は変えない**ので、
    //     「申込」を書いてはいけなくなるわけではない（書いてよい場面では今までどおり通る）。
    direction: "急かさない受け止め＋次工程（内覧／ご不明点）の開放1文。「はい😊！！ごゆっくりご確認頂けますと幸いです！！」→「お部屋お気に召されましたら実際にご案内させて頂きますのでいつでもお気軽にご連絡ください😌！！」。70〜130字",
    mustInclude: [
      { label: "急かさない受け止め（顧客が使った動詞をそのまま鏡写しにする）", detect: GENTLE_WAIT_RE, when: hasGoyukkuriMirrorVerb,
        fix: "顧客が『確認します』なら「ごゆっくりご確認頂けますと幸いです😊！！」／『検討します』なら「ごゆっくりご検討頂けますと幸いです😊！！」" },
      { label: "次工程の開放（内覧のご案内／ご不明点）", detect: /お気に召され|ご不明|ご不安|いつでも/,
  ...
  // 2026-09-12 竹内「見積書を送った後は申込へではない」: 旧セルは「申込打診」で申込でお部屋を抑える宣言を必須にしていた。
  //   実データ（見積送付後の前向き返答へのスタッフ返信）は「お気に召されたお部屋ご都合よろしいお日にちにご案内させて頂きます」等の
  //   内覧のご案内が中心で、見積書の次に申込へを押したのは185件中18件（10%）。申込の話はお客様が自分から申込を口にした時だけ
  { id: "ES_POSITIVE", staff: "estimate_send", customer: "positive", precedence: "after_wait",
    tpoLabel: "見積送付後の前向き返答（内覧のご案内）",
    // 2026-09-20 竹内（まりあさん事例）: 旧 fix / example は「ご都合よろしいお日にちに」をリテラルで持っており、
    //   内覧が既に決まっている会話では行動台帳の「新しい日程は打診しない」と正面衝突していた（書くなと書けの同時指示）。
    //   {viewingOffer} 1つに寄せて、決まっている時は台帳の日時をそのまま言う形（「明日16時お部屋ご案内させて頂きます！！」）に切り替える。
    direction: "見積への前向き反応を受け止め、「{viewingOffer}」の1文。申込へは進めない（お客様が自分から申込を口にした時だけ申込の話をする）。煽り禁止。70〜140字",
    mustInclude: [{ label: "内覧のご案内", detect: /ご案内|ご内覧|内覧|内見/,
      fix: "「{viewingOffer}」を1文入れる" }],
    mustNot: ["お客様が申込を口にしていないのに申込を促す", "受け取っていない申込・書類を「受け取りました」と書くこと", "既に決まっている内覧があるのに新しい日程を打診すること（「ご都合よろしいお日にち」）", "「埋まってしまいます」等の煽り", "書類リスト生成", "未確認の退去日・入居可能日の断言"],
    example: "はい！！\n{viewingOffer}",
    length: "70〜140字", closer: "none", nanisotsu: false },
  ...
  { id: "PS_CONCERN", staff: "property_send", customer: "concern", precedence: "override_wait",
    ...
    example: "{name}お世話になっております！！\nご要望お聞かせ頂きありがとうございます😊！！\nお風呂広めのお部屋を中心に{name}にオススメできるお部屋お調べさせて頂きます！！\n{name}にご満足頂けるお部屋が見つかるまで全力でサポートさせて頂きます！！",
  ...
  { id: "PS_THINKING", staff: "property_send", customer: "thinking", precedence: "after_wait",
    ...
    mustInclude: [
      ...
      // 2026-09-20: 「何時でも」「お気軽に」「ご連絡お待ちしております」も扉を開ける1文（OPEN_DOOR_RE）
      { label: "随時ピックアップ宣言 or 扉を開ける1文", detect: DOOR_OPEN_ANY_RE,
  ...
  { id: "PS_WILL_SEND", staff: "property_send", customer: "will_send_later", precedence: "override_wait",
    ...
    direction: "我々が物件を送った後、お客様が「気になる物件を（後日）送る」と予告した、または「他社で内覧した・気に入った物件があって初期費用を知りたい」とまだ送っていない物件の見積を頼んだ。①開口語「はい！！」または「かしこまりました！！」（『可能でしょうか』と尋ねられた時は「かしこまりました😊！！」）②（お客様が『送っていいですか』と許可を求めた時だけ）「気になるお部屋ございましたらいつでもお送りください😊！！」③【核】お送り頂き次第の業務フロー1文＝募集状況確認＋最大限割引した初期費用の御見積書（まだ送っていない物件の見積の依頼には「{name}お気に召されましたお部屋お送り頂きますと最大限割引しました初期費用御見積させて頂きます！！」の形）。物件はまだ届いていないので「お送り頂きました物件の〜確認させて頂きます」と届いた前提で言い切らない。我々が探して送る話（随時ピックアップ）はこの場面では書かない。90〜160字",
    mustInclude: [
      { label: "送られた物件の募集状況を確認する宣言", detect: WILL_SEND_RECEIVE_CHECK_RE, when: isPropertyForecast,
    ...
    mustNot: ["「随時ピックアップしてお送りします」（我々が探す宣言。顧客が物件を送った直後の正解 246件中 0件・(B)場面専用の語彙）", "顧客が『相談』と言っていないのに「ご相談」（正解 (A) 28件中 0件）", "顧客が『検討』『確認』と言っていないのに「ごゆっくり」", "申込誘導", "希少性煽り", "「かしこまりました！！」単独終了", "まだ届いていない物件を届いた前提で言い切る（「お送り頂きました物件の初期費用確認させて頂きます」）"],
  ...
  { id: "PS_QUESTION", staff: "property_send", customer: "question", precedence: "override_wait",
    ...
    // 2026-09-20 竹内「AIX の機能も把握したうえで調査」（scripts/audit-aix-territory.ts）:
    //   この example は「すぐ埋まってしまう可能性が高い」を含む＝ AIX【申込へ！】の forbid「希少性の煽り」。
    //   旧ゲートは /内覧|内見|見たい/ を含み、お客様が「内覧したいです」と言っただけで煽りの手本が出ていた
    //   （examplePremise は「押さえ方を質問され」なのでゲートの方が広かった）。
    //   実送信は「埋まってしまう」が12,000通中38通（0.3%）＝ スタッフはほぼ書かない。
    //   設計知見「入口は厳しくてよい／出口は誤削除0でなければ入れない」により、**入口（ゲート）だけ**前提に合わせる。
    //   「初期費用を抑えたい」では当てない（「お部屋を押さえる」の意味に限る）。
    exampleRequires: /埋ま|人気|(?:お部屋|物件|部屋)[^\n]{0,8}(?:押さえ|抑え)|押さえ(?:方|られ|たい|て)|キープ/,
    // 前提が立たない普通の質問には、煽りも「押さえ」も含まない型だけを見せる（direction の「不明なら確認宣言」側）
    exampleFallback: "{name}お世話になっております！！\nこちらのお部屋の詳細につきましては、確認出来次第ご連絡させて頂きます！！\n何卒よろしくお願い致します！！",
  ...
  { id: "QC_ANSWER", staff: "question_to_customer", customer: "*", precedence: "after_wait",
    ...
    mustInclude: [
      ...
      // 2026-09-20 竹内「次の一手の必須を緩める」: 次工程の語彙が狭すぎて、スタッフの実送信を
      //   取りこぼしていた（44件中 14件しか満たさない＝32%）。実物の次工程はもっと広い:
      //     「10時半頃のお電話お待ちしております😊！！」
      //     「マンション下に到着時間分かりましたらご連絡させていただきます😌！！」
      //     「ただいまからお申込みフォームお送りさせていただきます！！」
      //   どれも「回答を前提にした次工程」なので認める（必須は外さず、実文を通す）
      { label: "回答を前提にした次工程宣言", detect: new RegExp(`(?:御見積書|お見積書|内覧|ピックアップ|確認|ご案内|ご連絡|お送り|お申込|手続き|お調べ|交渉).{0,30}${DECL_TAIL}|お待ちしております|お待ちし(?:て|ており)ます`),
  ...
  { id: "ANY_WILL_SEND", staff: "*", customer: "will_send_later", precedence: "override_wait",
    tpoLabel: "持込予告",
    direction: "お客様が『自分で気になる物件（または条件）を送る』と予告した、または「他社で内覧した・気に入った物件があって初期費用を知りたい」とまだ送っていない物件の見積を頼んだ。PD_WILL_SEND と同じ骨格。①開口語「はい😊！！」（『可能でしょうか』と尋ねられた時は「かしこまりました😊！！」）②【核】お送り頂き次第の業務フロー1文（物件＝募集状況確認＋最大限割引の御見積書／条件＝条件に合うお部屋を探す。まだ送っていない物件の見積の依頼には「{name}お気に召されましたお部屋お送り頂きますと最大限割引しました初期費用御見積させて頂きます！！」の形）。物件はまだ届いていないので「お送り頂きました物件の〜確認させて頂きます」と届いた前提で言い切らない。我々が探して送る話（随時ピックアップ）・お客様が言っていない「ご相談」「ご検討」は書かない。80〜160字",
    mustInclude: [
      { label: "送られた物件の募集状況を確認する宣言", detect: WILL_SEND_RECEIVE_CHECK_RE, when: isPropertyForecast,
        fix: "「お送り頂きました物件の募集状況確認させて頂きます！！」を1文入れる" },
      { label: "最大限割引した初期費用の御見積書の予告", detect: WILL_SEND_ESTIMATE_FORECAST_RE, when: isPropertyForecast,
    ...
    mustNot: ["「随時ピックアップしてお送りします」（(B)『我々に送って』場面専用の語彙。(A) では正解 0/246）", "顧客が言っていない「ご相談」", "顧客が言っていない「ごゆっくり」「ご検討」", "申込誘導", "希少性煽り", "「かしこまりました！！」単独終了", "まだ届いていない物件を届いた前提で言い切る（「お送り頂きました物件の初期費用確認させて頂きます」）"],
    example: "お世話になっております！！\nはい😊！！\n気になるお部屋ございましたらいつでもお送りください！！\nお送り頂きました物件の募集状況確認させて頂き、最大限割引しました初期費用の御見積書とあわせてご連絡させて頂きます！！",
    examplePremise: "お客様が『気になる物件を後日送る』と予告した場合（このセルの条件そのもの）",
    length: "80〜160字", closer: "none", nanisotsu: false },
  ...
  { id: "AD_ANY", staff: "apply_done", customer: "*", precedence: "after_wait",
    ...
    mustInclude: [
      // 2026-09-20 竹内「返信が実際送ったのと比べてまちがったのが出る可能性ないか」の調査で直した:
      //   受け止めだけを必須にしていたため、**お客様の質問への回答**の場面で満たせなかった（実送信で 2/5）。
      //   実物「1番手お申込み中の方がキャンセルもしくは審査否決の場合に2番手お申込み者の方が1番手に
      //   繰り上がり審査開始となります！！」＝受け止めで始まらないのが正しい。SW_ANY と同じ形に揃える。
      { label: "受け止め、または質問への回答",
        detect: /はい[！!😊]|かしこまりました|承知(?:いた)?しました|ありがとうございます|となります|ございます|(?:いた|し)ます[！!。]/,
        fix: "質問があれば履歴にある事実で「〜となります！！」と答え、無ければ「はい😊！！」「かしこまりました！！」で受け止める" },
    ],
    mustNot: [
      "審査の結果・通過の可否・かかる日数の断言",
...
export function fillPairTokens(s: string, pair: PairContext): string {
  ...
  return s
    ...
    // 2026-09-20 竹内（まりあさん事例）: 内覧が決まっている時は台帳の日時をそのまま言う形に切り替える。
    //   ここが {viewingOffer} の唯一の展開点なので、これを見る全セル（ES/PS/CR_POSITIVE …）が同時に正しくなる（四者同名）
    .replace(/\{viewingOffer\}/g, viewingOfferLiteral(pair.customerName, pair.namedProperty.matchedSent, pair.namedProperty.count, pair.ledger?.facts.viewingAppointment ?? null))
