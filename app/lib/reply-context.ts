...
export const PAIR_MATRIX: PairRule[] = [
  ...
  { id: "VI_POSITIVE", staff: "viewing_invite", customer: "positive", precedence: "after_wait",
    ...
    direction: "内覧受付。行動台帳に決まっている内覧があればその日時の復唱のみ（新しい日程は聞かない）。無い場合は、候補日時は AIX 内覧日調整で送るため本文は受付＋「ご都合よろしいお日にち御座いますでしょうか」の確認のみ。既に候補日を出している場合は日時確定の復唱のみ。40〜90字",
    mustInclude: [
      ...
      { label: "日程確認 or 確定復唱", detect: /ご都合|お日にち|[0-9０-９]{1,2}[/／月][0-9０-９]{1,2}|[0-9０-９]{1,2}時/,
        ...
        fix: "行動台帳に決まっている内覧があればその日時をそのまま復唱する。無い場合は、候補日時を出していなければ「{name}ご都合よろしいお日にち御座いますでしょうか😊！！」、出していれば直前スタッフ発言の日時をそのまま復唱する（日時を作らない）" },
    ],
    mustNot: ["申込誘導", "別物件提案", "募集未確認物件への内覧確約"],
    example: "かしこまりました！！\nお部屋ご案内させていただきます！！\n{name}ご都合よろしいお日にち御座いますでしょうか😊！！",
