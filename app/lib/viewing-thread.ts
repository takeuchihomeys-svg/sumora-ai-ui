...
/**
 * こちらの「内覧後のお礼」＝内覧が終わった確かな印。
 * 2026-09-21 竹内（まりあさん事例）: 行動台帳もこれを見る（export）。台帳は待ち合わせを**日付だけ**で見ていて、
 *   16:00 の内覧が終わりお礼まで送った 21:15 にも「この内覧は決まっている・本日16時ご案内」と渡していた
 *   → 生成が内覧前の例文（明日お会い出来るのを楽しみに…）を写した。同じ事実を別々に判定しない（1か所）
 */
export const STAFF_VIEWING_DONE_RE = /(?:本日|昨日)(?:は)?(?:お時間|ご内覧)(?:を)?(?:頂|いただ)き(?:まして)?ありがとうございました/;
