...
(function (root, factory) {
  ...
  function behavior(mode, brain) {
    ...
    return {
      ...
      brainDrop: b && !staff,
      // 2026-09-25 竹内「共有しておく」: スタッフ × ブレインでも売上番長グループの説明文の末尾に「🧠 ブレイン判定 …」の1ブロックを付ける
      //   （スタッフは外さないので、まとめは「外す候補」の言い方＝サーバーの apply_drop=false のまま）。お客様に届く文ではない
      brainNote: b,
      recordPickup: b,
      // 2026-09-25 竹内「スタッフモードで送った時は、お客さんのところ完了ボタン押したら、リアプロと itandi の全部分析されるようにする」:
      //   お客様の作業を終えた時（一覧の「確認」☑／リアプロの「✅ 送った」）に /api/property-pickups/complete を呼び、
      //   売上サポの回（リアプロ・itandi・レインズ・自動送信の回も）を1つにまとめて読み・順位・👑 を付け直す。
      //   ブレイン×スタッフは必ず・ブレイン×通常／AIX も押した時はまとめる。ブレイン OFF は売上サポに届いていないので呼ばない
      // 2026-09-25 v2.5.23 10分の自動まとめ（background._schedulePickupIdle）も同じ completeGroup で呼ぶ（送信から10分半後・idle:true）。
      //   「完了」はすぐまとめたい時用に残す。PC が消えていてもサーバーの Cron（2分おき）がまとめる
      completeGroup: b,
      ...
      claimBrainCommands: b && !staff,
      // 2026-09-25 竹内「ブレインモードで物件自動検索や一括検索した際に、検索がちゃんとされていなかったら原因を見つけられるようにする」:
      //   検索の1回ごとに「入れようとした条件・実際に入った値・結果」を /api/search-audits に送るか（search-audit.js）。
      //   ブレインの時だけ（スタッフ×ブレインの手の検索も含む）。ブレイン OFF は送らない（サーバーも brain!==true なら何もしない）
      searchAudit: b,
    };
  }
  ...
  function banner(mode, brain) {
    ...
    if (mode === "staff") {
      return b
        ? { cls: "brain-staff", text: "🧠 ブレイン × スタッフモード — 自動化は停止中（手動のみ・2時間で自動OFF）。手で送る物件もブレインが判定しAIXツールに記録します（物件は外しません）。ウェブの一括検索は拾いません（別のブレインのPCが拾う）" }
        : { cls: "staff", text: "スタッフモード中 — 自動化は停止しています（手動操作のみ・2時間で自動OFF）" };
    }
    if (mode === "aix") {
      return b
        ? { cls: "brain", text: "🧠 ブレイン × AIX連動 — AIXの指示に連動して自動検索し、お客様の条件・過去の物件出し・利益（AD−割引）で判定してから売上番長グループへ送ります。AIXツールの一括検索も受け取ります（11:00／17:00 の自動便は見送り）" }
        : { cls: "aix", text: "AIXモード中 — AIXの指示（物件ピックアップ・物件オススメ）に連動して自動で物件検索し、売上番長グループへ送ります" };
    }
    return b
      ? { cls: "brain", text: "🧠 ブレイン × 通常 — 送る物件をお客様の条件・過去の物件出し・利益（AD−割引）で判定してから売上番長グループへ送り、AIXツールに記録します。AIXツールの一括検索も受け取ります" }
      : null;
  }
