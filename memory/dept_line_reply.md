...
- **中身**: `app/lib/property-send-match.ts`（糸口の抽出＝前回の送付より後のお客様の気にしていること／こちらの約束・経緯、続いている事情は前回より前からも／内覧誘導と日時の行の除去／前の発言のお礼の繰り返しの除去／手本6通・テスト8件）、aix/action property_send の `conversation_match` 分岐（構成①〜⑤・③は糸口の事柄だけ・数字は入力に無ければ〇〇）
- 確認: カイナ（9/15 19:05 の直前）本番3回 → ②は「エリア広げさせていただき、大きめのお部屋で…」（実送信と同じ言い方）。前日のお礼の繰り返し・代理契約の一文の欠けを直した（再確認は次の本番）
