...
## 一覧の並び順（竹内・2026-09-18）— 黄金ルール

- 竹内①「**必ずのお客さんはLINEの上に上がるようにする。忘れないようにする為**」
- 竹内②（①の直後の修正）「**メッセージがきているお客さんで時間最近の方が上に配置。この必ずは、メッセージが来ていない中なら、メッセージ来ていない中で上。メッセージ来ているなら、メッセージ来ている中で上にする**」
- **最終的な並び**（`app/lib/conversation-order.ts` の `compareConversationOrder`）

  1. **メッセージが来ている（返事待ち＝最後の発言がお客様）が上**
  2. その**組の中**で【必ず】（お客様への約束・未履行）が上
  3. 【必ず】どうしは**約束が古い順**（放置が長いほど上）
  4. 残りは従来どおり直近やり取り順

- **やらかし（同日中に修正）**: 最初の実装は【必ず】を**一覧全体の先頭**に出していた。それだと**今まさに返事を待っているお客様が、約束だけの会話の下に押し下げられる**。一覧の第一の目的は「お客様を待たせない」なので、**約束は順位を飛び越えず、それぞれの組の中で上げる**。
- **本番の実データでの効き方**（9/18・未履行の【必ず】9件／8会話）
  - 📩来てる組: 名無しの権兵衛（旧45位→1位）・𝒮❦（旧26→2）・🐈‍⬛（旧42→3）・💜（旧37→4）・りな（旧38→5）
  - 来てない組: 慶次（旧32→組の1位）・YUYA（旧28→2）・まりあ（旧3→3）
  - 来てる組の先頭5人はすべて【必ず】持ち。**約束のある人が全員、自分の組の一番上に来た**
- **判定の置き場所**（規則を2か所に書かない）
  - `conversation-order.ts` … 並びそのもの（`compareConversationOrder` / `hasIncomingMessage` / `sortMsOf`）
  - `promise-calendar.ts` … 約束の材料だけ（`oldestPromiseAtMs` / `promiseOverdueDays`）。**バッジの日数と並びは同じ材料**（表示と順番が食い違わない＝四者同名）
- **並びは全部の絞り込みに効く**（すべて／要対応／AIX／ステータス／名前検索）。AI検索の結果だけは従来どおり関連度順（早期 return のまま）
- **テスト**: `app/lib/__tests__/conversation-order.test.ts` 12件（新規）＋ `promise-calendar.test.ts` 18件。tsc 0 ／ lib テスト97ファイル全 PASS
- **未確認**: 本番の画面での見え方（竹内さんの目視待ち）

---

## （旧・記録）一覧で【必ず】のお客さんを一番上に（竹内・2026-09-18・上の項目で置き換え済み）
