...
- **【続き】入口だけ直しても生成後の癖は残る — 出口の決定論も配る**（コミット bd01c4cd）
  ...
  - **残っている課題**: 実送信0件の創作（「重複しないよう」「内覧・お申込みどちらからでも」等）は語ごとの出口では追いきれない。ピックアップ文の実送信738件では「お客様名あり62%」「お手隙の際にご査収40%」が型なので、**型との距離を測る仕組み**が要る（`aix_generate_log` に残るようになったので、これから測れる）
