...
- **残**: ①画像だけの送信（send-line-message に message なし）は記録されず、画像でピックアップを届けた分（30日で 231件中 12件）はピックアップの【必ず】が残る→手で済みか「画像送付あり」の注記が次の一手 ②会話画面の「やること」バナーに【必ず】を出す導線は未実装 ③直近3日の未履行の約束 9件・慶次の保証会社の確認の遡り登録は竹内さんの判断待ち ④初週は朝の報告の🔴件数と手動「済み」の頻度を見て語彙（BARE_CONFIRM_DECL_RE・CONDITIONAL_PROMISE_RE）を調整する
