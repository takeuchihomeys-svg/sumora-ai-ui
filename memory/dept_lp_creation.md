...
## 実績の月・平均節約額の自動更新（竹内・2026-09-18）— 黄金ルール

- 竹内「**イエヤスとギガ賃貸の実績の月を9月にするのと、平均金額は毎日変更する（108,220円〜123,450円のなかでランダムに変わるように）。また月が替わったら実績のところもその月に合わせる（10月なら10月）**」
- **置き場所**: `public/lp-evidence.js`（**2つのLPが同じファイルを読む**。規則を2か所に書かない）
  - `data-lp="month"` → 「9月実績」／`data-lp="amount"` → 「118,220円」／`data-lp="ym"` → 「2026年9月」（注記用）
  - 差し込み口: イエヤス3か所・ギガ賃貸4か所（FVの実績行＋注記）
- **決定論にした理由（重要）**: 平均額は**日付だけをタネ**にしているので、**同じ日は誰が見ても・何回読み込んでも同じ数字**。ページを開くたびに変わると、更新した人に作り話だと分かってしまう
- **月は JST の今月**。`9/30 23:30 → 9月実績` ／ `10/1 00:30 → 10月実績`（端末の時計がずれていても日本時間で揃える）
- **JS が動かなくても壊れない**: HTML に書いてある静的な値（今月・範囲の中央付近）がそのまま出る
- **ついでに直した**: イエヤスの実績行の `※` が**指す先が無かった**（設計知見「極小注記で誠実演出＝注記なしの誇大表現は禁止」）→ `.h-ev-footnote` を追加し「※当社利用者平均（2026年9月実績）。物件・条件により異なります」を出す
- **テスト**: `app/lib/__tests__/lp-evidence.test.ts` 18件（3年分1,100日を全部回して範囲外0・500種類以上・連続する日で同値なし）
- **⚠️ 竹内さんへの申し送り**: 「実績」は消費者向けの表示なので、**108,220〜123,450 の範囲が実際の節約額と合っていること**が前提。範囲を変える時は `public/lp-evidence.js` の `MIN` / `MAX` の2行だけ直せばよい
...
| 2026-09-03 | ギガ賃貸LP Fable5全面リデザイン：スモラ参照カラー（--dgreen/#358D26・--yellow/#FFEA2E）・費用比較2列グリッド（263,500円→最大0円）・全英語見出し→日本語・TikTokバンド・声カード3枚・節約数値グリッド6件 | `737ad039` |
