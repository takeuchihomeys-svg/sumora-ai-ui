...
## 2026-09-27 v2.5.29 待ち時間を人のようにばらつかせる（human-wait.js）・検索 → 並び替えの間を足した
竹内「拡張ツールで物件検索を押してから並び替えを変える所、毎回一定の時間になっているか？なっていたら全て不規則にする（時間ランダム）。拡張ツールは人間らしい動きをするために全て時間ランダムにする」
- **調べた事実**: 待ちは全部で約230か所。page-script／itandi-page-script／itandi-bulk-dl の人の操作（クリックの前後・区や駅を1件ずつ）は**すでに大半が乱数**（700+rand×400 等・クリックの列 humanRand）。固定の数字は140か所で、うち人の操作・画面の落ち着き待ちが約60か所。
  **検索 → 並び替え**（リアプロ bulk-dl の AD 高→低）は、結果が出てから 0.2秒（固定・予備の道）か 0.6〜1.2秒で autoSendAllPages → **すぐ location.href で並び替えのリンクへ遷移**（間が 0）だった。itandi・レインズには並び替えの操作は無い（page-script の 17時便の並び順は queueSelVal＝乱数の列で既にばらつく）
- **共通の関数** `chrome-extension/human-wait.js`（UMD・self.AxlxHumanWait・mode-core.js と同じ型）:
  - `humanDelay(ms)` 人の操作の間: base×0.8〜1.5（平均 1.15倍）。50ms 以下は元より短くしない・2秒を超えたら上の幅を min(0.5倍, 1秒+0.1倍) に狭める（10秒 → 8〜12秒）
  - `settleDelay(ms)` ページ・選択肢・モーダルが落ち着くのを待つ固定の秒数: **元より短くしない**（base〜1.35倍・大きい値は同じく狭める）＝速くして壊さない
  - `pollDelay(ms)` 条件を見る間隔: ±15%（平均は同じ＝回数で打ち切る待ちの長さは平均で変えない・500ms×12回の最長 6.9秒）
  - Promise 版 humanWait/settleWait/pollWait
- **読み込み**: background（import）・popup.html（先頭）・manifest の先頭の段（document_start・3サイト）を `["human-wait.js","search-audit.js"]`・itandi のページの中の段（world:MAIN）を `["human-wait.js","itandi-page-script.js"]`・web_accessible_resources に追加。**page-script.js／reins-page-script.js（ページの中）** は content.js／reins-content.js が `<script>` を **async=false** で human-wait.js → page-script の順に入れる。ページの中の物は「読めない時は元の値」の予備を持つ（止まらない）
- **置き換えた所**（固定 → ばらつき）:
  - bulk-dl: **並び替えの前に `_hd(900)`（0.72〜1.35秒・旧 0）**。状態 sortState は待つ前に書く・待つ前に `_pendingAutoSendDispatched=true`（待つ間に Case B が並び替え前のページを送らない）・待つ間に止められたら遷移しない。結果 → 自動送信の開始 200ms（2か所）・次のページへ 800ms（2か所）→ `_hd`・Case D の 2500ms → `_sd`
  - page-script: waitForClick の見る間隔 500ms → `_pd`・省略時の pauseMs 600 → `_hd`・固定 300 → `_hd`・クリックの列が空くのを待つ 150/200 → `_pd`・市区のチェック 150 → `_hd`・「検索条件を表示」の 300/800/1800 → `_hd`
  - itandi-page-script: 確定の後の確認 1500 → `_sd`・近畿/大阪府の後 100・区ラベル 500・全選択 400・路線なし 300・最初の 800 → `_hd`・見る間隔 200/400 → `_pd`・リセット 600 → `_sd`・残ったチップを消す間（旧 i×250 の等間隔）→ 1件ずつ `_hd(250)` を足す・**ペットの欄 700 → `_sd`（0.70〜0.95）と検索を押す 1000 → `_sd`（1.00〜1.35）＝前後が入れ替わらない**
  - reins-page-script: 選択肢の読み込み待ち 600/800 の9か所 → `_sd`・ペットの後 300・検索の前 600 → `_hd`
  - reins-bulk-dl: 1件ずつ資料を取る間 1500・失敗の後 800・CB の後 400・確認の前 800 → `_hd`・DOM の落ち着き 600/300/200 → `_sd`
  - background: タブを開いた後／切り替えた後の 1500/2000/2500/500・次のページの 2000・`alive ? 300 : 1500` → `_settleMs`・フリーワード検索の見る間隔 500（3か所）→ `_pollMs`
  - content.js: 自動入力を始める 500 → `_hd`・fill を送る 1000 → `_sd`・サイドバーの 200/600/1500 → `_hd`／reins-content 200/600・underbar の転送 1000・popup の見積タブ 300 → `_sd`・popup のお客様の読み込み待ち 100 → `_pd`・itandi-bulk-dl の PDF ボタン待ち 250 → `_pd`
- **残した物（80か所・テストの表 KEEP に理由付きで固定）**: 見張り（85秒・240秒・90秒）・期限（abort・35秒の判定・PDF 60秒）・サーバーのやり直し・Realtime の心拍と再接続・setInterval の見る間隔・MutationObserver の間引き・checkbox の差し込み・画面の表示を消す・起動時・Case C の予備の起動・既に乱数の所（人の間 3〜8秒・地域→駅 5〜10秒 等）
- **速さ**: 1人あたり リアプロ 約 +1.3〜1.6秒（並び替えの前 +1.0秒が大半）・itandi 約 +0.8〜1.2秒・レインズ 約 +1秒＋物件1件ごとに +0.2秒。1人の検索〜送信は30〜60秒なので **約3〜5% 遅くなる**（30人の一括で +30〜45秒）
- テスト: `tests/chrome-extension/human-wait.test.js`（205・範囲・配線・置き換え漏れ＝固定の数字の待ちが KEEP の表と一字一句同じ件数・並び替えの順・前後の入れ替わり・5大バグの型・全ファイル node --check）。search-audit.test.js の配線の1件を「先頭の段が human-wait.js → search-audit.js」に直した。既存の拡張テスト全部・app 側の拡張を読むテスト7本・tsc 0
- **竹内さんが確かめること（2.5.29 に再読み込み後）**: ①リアプロで一括検索 → 結果が出てから少し間を置いて（約1秒）AD 高→低の並び替えに進む ②コンソールに `[AXLX bulk-dl] AD高→低ソート適用` の後に遷移 ③itandi でペット相談のお客様 → ペットの欄が付いてから検索が押される ④ページのコンソールで `window.AxlxHumanWait`（リアプロ・レインズのページ）が見える（無くても元の値で動く）
- **新しい待ちを足す時**: 人の操作なら `_hd`／落ち着き待ちなら `_sd`／見る間隔なら `_pd`。固定の数字で残すならテストの KEEP に理由を書いて足す（足さないとテストが落ちる）
...
## 2026-09-26 AIXツール: 10件を超えたチェック・分析を「まとめ」で1回に（拡張のコードは変えていない）
- **既定のチェック**（`pickup-review-order.defaultAixChecks`）: まとめの回ごとに点の高い順（👑 先頭・外す候補／送った物／72時間切れは除く）で **10件まで**。旧は外す候補以外を全部＝リアプロの一括（2件ずつ7回・20件・外す候補0）で「AIX物件ピックアップ（20件）」になり、押すと「10件までに」で止まっていた
- **10件を超えた時**: ボタンが「⚠️ 20件チェック中 → 点の高い10件に絞る」。押すと点の高い10件（`pickTopForAix`・画面の並び sortForReview と同じ）にチェックを絞り、もう一度押すと AIX に移る。分析の吹き出しの「点の高い3件だけ選ぶ」は**画像で分析の点**の上位3件（別物・そのまま）
- **自動まとめ 10分 → 3分**（`AUTO_COMPLETE_QUIET_MINUTES`）: 実測（`scripts/audit-pickup-arrivals.ts`・9/24〜26 本番 16回・6ラウンド）で分かれて届く間隔は 6〜23秒が9/10・最大122秒。Cron 2分おきなので実際は最後の物件から3〜5分。拡張の alarm（10分半）はそのまま（鳴っても「もうまとめてある」）
