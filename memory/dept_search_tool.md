...
| 条件 | 中身 |
|---|---|
| ① 直近3日に物件出しした人 | `RECENT_SENT_DAYS=3`。**JSTの日付で数える**（時刻差だと3日前が1時間の違いで外れる） |
| ② 新規のお客さん | 登録から `NEW_CUSTOMER_DAYS=14` 日以内で**まだ一度も送っていない**人 |
| 除外 | 申込以降・終了・保留（`applying / closed_* / cancelled / pending`）、エリア条件が無い人 |
| 上限 | `MAX_TARGETS_PER_RUN=40`（hot → 直近送付 → 新規 の順に切る） |

**② で登録日を見る理由（実データ 2026-09-19）**: まだ送っていない人は109人いるが、
**1か月より前の登録が98人**（うち80人が property_search）＝古い放置客。毎日回す相手ではない。
14日で切ると対象は **27人**（直近送付22＋新規5）に落ち着く。

### どの条件で検索するか

| | 11:00（am） | 17:00（pm・最新物件） |
|---|---|---|
| 更新日 | **前回送った日から計算**（昨日→1日以内／3日前→3日以内。拡張 `calcUpdateDays` と同じ線） | **1日以内（本日の更新日付）** |
| 並び | AD高い順 | **更新順** |
