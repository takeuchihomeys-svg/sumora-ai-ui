...
- **itandi自動入力・所在地/路線・駅選択モーダル・検索ボタン自動クリックが実装済み（v1.5.0）**
- フロー: 路線・駅で絞り込み → 近畿 → 大阪府 → 路線チェック（600ms待） → 駅列描画待ち（800ms） → 駅を部分一致選択 → 確定
- 駅名は「駅」サフィックスを除去してから検索（「堺筋本町駅」→「堺筋本町」）
- 駅選択は `textMatch`（`includes` 部分一致）を使用。完全一致不要
- タイミング（setTimeout）が合わない場合は値を調整すること（800ms/600ms等）
- レインズ手順整備が次の大きな課題
