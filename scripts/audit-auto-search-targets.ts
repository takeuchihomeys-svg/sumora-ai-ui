...
import { selectAutoSearchTargets, rpUpdateDaysFor, MAX_TARGETS_PER_RUN } from "../app/lib/auto-search-schedule";
...
async function main() {
  ...
  const rows = (data ?? []) as Array<{
    ...
    last_property_sent_at: string | null; created_at: string | null;
  ...
  console.log(`  ・直近3日に物件出しした人 : ${recent.length} 人`);
  console.log(`  ・まだ一度も送っていない人 : ${fresh.length} 人`);
  ...
  for (const t of capped.slice(0, 10)) {
    ...
    const days = rpUpdateDaysFor(c?.last_property_sent_at ?? null);
    console.log(`  ${(c?.customer_name ?? "?").padEnd(12)} status=${String(c?.status).padEnd(16)} 理由=${t.reason.padEnd(12)} 更新日=${days ?? "なし"}`);
  }
}
