...
・保証会社の審査難易度と種類【重要】:
  種類は3つ（独立系・信販系・信用系）。LICC に加盟している会社（全保連・ジェイリース等）は信用系（2026-09-26 竹内さん・「LICC系」という種類は使わない）
  審査の厳しさ: 信販系（最も厳しい）> 信用系（一般的）> 独立系（最も緩い）
  - 信販系（エポス・オリコ等）: クレジットカードや信用情報（CIC）を確認。過去のクレカ滞納があると否決になりやすい
  - 独立系（日本セーフティ・いえらぶ保証・Casa等）: 独自審査基準で審査基準が最も緩い。信販系・信用系否決後の受け皿
  - 信用系（全保連・ジェイリース・K-net等）: 金融系の情報ではなく、過去の家賃滞納やトラブルが無かったかを見る（信販系とは別の種類・2026-09-26 竹内さん）
